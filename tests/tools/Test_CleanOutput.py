...
class TestCleanOutput(unittest.TestCase):
    def setUp(self):
        self.test_dir = os.path.join(Informer.unit_test_build_path(), "tools",
                                     "CleanOutput")
    ...
    def test_clean_output(self):
        with self.assertRaises(MissingExpectedOutputError):
            clean_output(input_file=self.input_file_path,
    ...
    def test_cli(self):
        ...
        with open(self.volume_file_path, "w"):
            pass
        result = runner.invoke(clean_output_command,
                               ['-o', self.test_dir, self.input_file_path])
