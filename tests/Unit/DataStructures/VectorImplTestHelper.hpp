...
void vector_test_construct_and_assign(
    typename tt::get_fundamental_type_t<ValType> low =
        typename tt::get_fundamental_type_t<ValType>{-100.0},
    typename tt::get_fundamental_type_t<ValType> high =
        typename tt::get_fundamental_type_t<ValType>{100.0}) noexcept {
  MAKE_GENERATOR(gen);
  ...
  const auto generated_val1 = make_with_random_values<ValType>(
      make_not_null(&gen), make_not_null(&dist));

  const VectorType value_size_constructed{size, generated_val1};
  CHECK(value_size_constructed.size() == size);
  alg::for_each(value_size_constructed, [
    generated_val1, &value_size_constructed
  ...
  const auto generated_val2 = make_with_random_values<ValType>(
      make_not_null(&gen), make_not_null(&dist));
  const auto generated_val3 = make_with_random_values<ValType>(
      make_not_null(&gen), make_not_null(&dist));
...
void vector_test_serialize(
  ...
  const auto start_val = make_with_random_values<ValType>(make_not_null(&gen),
                                                          make_not_null(&dist));
  const auto val_diff = make_with_random_values<ValType>(make_not_null(&gen),
                                                         make_not_null(&dist));
  ...
  const auto generate_series = [&current_val, val_diff ]() noexcept {
...
#pragma GCC diagnostic pop
  ...
  }
  {
    ...
    const auto generated_val2 = make_with_random_values<ValType>(
        make_not_null(&gen), make_not_null(&dist));
    const auto sum_generated_vals = generated_val1 + generated_val2;
...
/// non-owning `VectorType` from an expression with `::ResultType` of
...
void vector_test_math_after_move(
  ...
  const auto generated_val2 = make_with_random_values<ValType>(
      make_not_null(&gen), make_not_null(&dist));
  const auto sum_generated_vals = generated_val1 + generated_val2;
  const auto difference_generated_vals = generated_val1 - generated_val2;

  const VectorType vector_math_lhs{size, generated_val1},
      vector_math_rhs{size, generated_val2};
  {
    ...
    detail::check_vectors(to_vector, VectorType{size, sum_generated_vals});
    ...
    detail::check_vectors(to_vector, sum_generated_vals);
  }
  {
    INFO("Check move assignment and value of target")
    auto from_val = make_with_random_values<ValType>(make_not_null(&gen),
                                                     make_not_null(&dist));
    VectorType from_vector{size, from_val};
    VectorType to_vector{};
    to_vector = std::move(from_vector);
    from_vector = vector_math_lhs + vector_math_rhs;
    detail::check_vectors(to_vector, from_val);
    detail::check_vectors(from_vector, sum_generated_vals);
  }
  {
...

  {
    ...
    from_vector = vector_math_lhs + vector_math_rhs;
    detail::check_vectors(to_vector, VectorType{size, from_val});
    detail::check_vectors(from_vector, VectorType{size, sum_generated_vals});
  }
}
...
struct VectorOrArrayAt {
  template <typename T, size_t S>
  auto operator()(const std::array<T, S>& container, size_t index) noexcept {
    return VectorOrArrayAt{}(gsl::at(container, index % S), index / S);
  }

  template <typename T,
            Requires<cpp17::is_arithmetic_v<typename T::ElementType>> = nullptr>
  auto operator()(const T& container, size_t index) noexcept {
    return container.at(index);
  }

  template <typename T, Requires<cpp17::is_arithmetic_v<T>> = nullptr>
  auto operator()(const T& value, size_t /*index*/) noexcept {
    return value;
  }
};
...
void vector_test_operator(const Functor& func,
                          const std::tuple<Bounds...>& bound_tup,
                          const std::tuple<Wraps...>& /*wraps*/,
                          const std::tuple<OperandTypes...>& /*operands*/,
                          std::index_sequence<Is...> /*meta*/) noexcept {
  MAKE_GENERATOR(gen);
  UniformCustomDistribution<size_t> sdist{2, 5};
  const DataVector used_for_size{sdist(gen)};
...
// enum for clarity in integral_constant below for choosing between execution
// paths when assembling the argument list for the math testing utility
enum VectorTestFunctorsChooser { FirstInplace, Continuing, Done };
...
struct next_choice_impl<true, IsFirstAssignment> {
  static const VectorTestFunctorsChooser value =
      VectorTestFunctorsChooser::Done;
};

// if we are assembling the first argument and the test type is Inplace, then
// treat it separately
template <>
struct next_choice_impl<false, true> {
  static const VectorTestFunctorsChooser value =
      VectorTestFunctorsChooser::FirstInplace;
};

// otherwise, continue assembling as ordinary
template <>
struct next_choice_impl<false, false> {
  static const VectorTestFunctorsChooser value =
      VectorTestFunctorsChooser::Continuing;
};

// helper function for easily determining the next branch of the call operator
// for VectorTestFunctorsImpl
template <typename VectorList, typename DistBoundList, TestType Test>
using next_choice = std::integral_constant<
    VectorTestFunctorsChooser,
    next_choice_impl<tmpl::size<VectorList>::value ==
                         tmpl::size<DistBoundList>::value,
                     tmpl::size<VectorList>::value == 0 and
                         Test == TestType::Inplace>::value>;
...
// `TestType::Inplace`, as inplace operators have specific demands on the
// first operand.
...
struct VectorTestFunctorsImpl {
  ...
  template <typename Functor, typename... DistBounds, typename... Wraps,
            typename... Vectors>
  void operator()(const Functor& func, std::tuple<DistBounds...>& bound_tup,
                  std::tuple<Wraps...> wrap_tup, std::tuple<Vectors...> vec_tup,
                  std::integral_constant<
                      VectorTestFunctorsChooser,
                      VectorTestFunctorsChooser::Done> /*done*/) noexcept {
  ...
  template <typename Functor, typename... DistBounds, typename... Wraps,
            typename... Vectors>
  void operator()(
      const Functor& func, std::tuple<DistBounds...>& bound_tup,
      std::tuple<Wraps...> wrap_tup, std::tuple<Vectors...> vec_tup,
      std::integral_constant<
          VectorTestFunctorsChooser,
          VectorTestFunctorsChooser::Continuing> /*continuing*/) noexcept {
    tmpl::for_each<WrapperList>([&func, &bound_tup, &wrap_tup,
      ...
      tmpl::for_each<UniqueTypeList>([&func, &bound_tup, &wrap_tup,
        ...
        VectorTestFunctorsImpl<Test, UniqueTypeList, VectorType0>{}(
            func, bound_tup, std::tuple_cat(wrap_tup, std::tuple<next_wrap>{}),
            std::tuple_cat(vec_tup, std::tuple<next_vector>{}),
            detail::next_choice<tmpl::list<Vectors..., next_vector>,
  ...
  void operator()(
      ...
      std::integral_constant<
          VectorTestFunctorsChooser,
          VectorTestFunctorsChooser::FirstInplace> /*first_inplace*/) noexcept {
    tmpl::for_each<NonConstWrapperList>([&func, &bound_tup ](auto x) noexcept {
      using next_wrap = typename decltype(x)::type;
      VectorTestFunctorsImpl<Test, UniqueTypeList, VectorType0>{}(
          func, bound_tup, std::tuple<next_wrap>{}, std::tuple<VectorType0>{},
          detail::next_choice<tmpl::list<VectorType0>,
                              tmpl::list<DistBounds...>, Test>{});
    });
  }
};
...
struct VectorTestFunctors {
 private:
  template <typename Functor, typename... DistBounds>
  void send_to_tests(
      std::tuple<Functor, std::tuple<DistBounds...>> fdata) noexcept {
    ...
    detail::VectorTestFunctorsImpl<Test, operand_type_list, VectorType0>{}(
        ...
        detail::next_choice<tmpl::list<>, tmpl::list<DistBounds...>, Test>{});
  }
 ...
 public:
  ...
  void operator()(std::tuple<FunctorsAndDistBounds...> func_tup) noexcept {
    tuple_fold(func_tup, [this](auto fdata) noexcept { send_to_tests(fdata); });
  }
};
}  // namespace VectorImpl
}  // namespace TestHelpers
