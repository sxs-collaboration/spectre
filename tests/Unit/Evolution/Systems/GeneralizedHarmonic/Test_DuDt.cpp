...
SPECTRE_TEST_CASE("Unit.Evolution.Systems.GeneralizedHarmonic.DuDt",
  ...
  tnsr::aa<DataVector, dim, Frame::System> dt_psi(n_pts);
  tnsr::aa<DataVector, dim, Frame::System> dt_pi(n_pts);
  tnsr::iaa<DataVector, dim, Frame::System> dt_phi(n_pts);

  const auto psi = create_tensor_with_random_values<
      tnsr::aa<DataVector, dim, Frame::System>>(n_pts, make_not_null(&gen));
  const auto pi = create_tensor_with_random_values<
      tnsr::aa<DataVector, dim, Frame::System>>(n_pts, make_not_null(&gen));
  const auto phi = create_tensor_with_random_values<
      tnsr::iaa<DataVector, dim, Frame::System>>(n_pts, make_not_null(&gen));
  ...
  const auto d_phi = create_tensor_with_random_values<
      tnsr::ijaa<DataVector, dim, Frame::System>>(n_pts, make_not_null(&gen));
  const auto gauge_function =
      create_tensor_with_random_values<tnsr::a<DataVector, dim, Frame::System>>(
          n_pts, make_not_null(&gen));
  const auto spacetime_deriv_gauge_function = create_tensor_with_random_values<
      tnsr::ab<DataVector, dim, Frame::System>>(n_pts, make_not_null(&gen));
  ...
  const auto lapse = create_tensor_with_random_values<Scalar<DataVector>>(
      n_pts, make_not_null(&gen));
  const auto shift =
      create_tensor_with_random_values<tnsr::I<DataVector, dim, Frame::System>>(
          n_pts, make_not_null(&gen));
  const auto upper_spatial_metric = create_tensor_with_random_values<
      tnsr::II<DataVector, dim, Frame::System>>(n_pts, make_not_null(&gen));
  const auto upper_psi = create_tensor_with_random_values<
      tnsr::AA<DataVector, dim, Frame::System>>(n_pts, make_not_null(&gen));
  const auto christoffel_first_kind = create_tensor_with_random_values<
      tnsr::abb<DataVector, dim, Frame::System>>(n_pts, make_not_null(&gen));
