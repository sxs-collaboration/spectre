...
void check_du_dt(const size_t npts, const double time) {
  ...
  auto box = db::create<db::AddSimpleTags<
      ...
      Tags::deriv<ScalarWave::Pi, tmpl::size_t<Dim>, Frame::Physical>,
      Tags::deriv<ScalarWave::Phi<Dim>, tmpl::size_t<Dim>, Frame::Physical>>>(
      Scalar<DataVector>(pow<Dim>(npts), 0.0),
      tnsr::i<DataVector, Dim, Frame::Physical>(pow<Dim>(npts), 0.0),
      Scalar<DataVector>(pow<Dim>(npts), 0.0),
      ...
      [&npts, &x, &time, &solution]() noexcept {
        tnsr::ij<DataVector, Dim, Frame::Physical> d2psi_dxdx{pow<Dim>(npts),
                                                              0.0};
        const tnsr::ii<DataVector, Dim, Frame::Physical> ddpsi_soln =
...
void check_normal_dot_fluxes(const size_t npts, const double t) {
  ...
  auto unit_normal =
      make_with_value<tnsr::i<DataVector, Dim, Frame::Physical>>(pi, 0.0);
...
void check_upwind_flux(const size_t npts, const double t) {
  ...
  // Any numbers are fine, doesn't have anything to do with unit normal
  tnsr::i<DataVector, Dim, Frame::Physical> unit_normal(pow<Dim>(npts), 0.0);
  for (size_t d = 0; d < Dim; ++d) {
    unit_normal.get(d) = x.get(d);
  }
  ...
  tnsr::i<DataVector, Dim, Frame::Physical> normal_dot_numerical_flux_phi(
      pow<Dim>(npts), 0.0);
