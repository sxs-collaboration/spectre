...
SPECTRE_TEST_CASE("Evolution.Systems.RadiationTransport.M1Grey.M1Closure",
  ...
  tnsr::i<DataVector, 3, Frame::System> momentum_density(used_for_size);
  tnsr::I<DataVector, 3, Frame::System> fluid_velocity(used_for_size);
  Scalar<DataVector> fluid_lorentz_factor(used_for_size);
  tnsr::ii<DataVector, 3, Frame::System> spatial_metric(used_for_size);
  // Output
  Scalar<DataVector> closure_factor(used_for_size);
  tnsr::II<DataVector, 3, Frame::System> pressure_tensor(used_for_size);
  Scalar<DataVector> comoving_energy_density(used_for_size);
  Scalar<DataVector> comoving_momentum_density_normal(used_for_size);
  tnsr::i<DataVector, 3, Frame::System> comoving_momentum_density_spatial(
      used_for_size);
