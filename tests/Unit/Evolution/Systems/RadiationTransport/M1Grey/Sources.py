...
def source_tilde_e(tilde_e,tilde_s,tilde_p,
                   source_n,source_i,lapse,
    ...
    result = (lapse * np.einsum("ab, ab",tilde_p,extrinsic_curvature) -
              np.einsum("ab, ab", inv_spatial_metric,
                        np.outer(tilde_s, d_lapse)) + source_n)
    return result


def source_tilde_s(tilde_e,tilde_s,tilde_p,
                   source_n,source_i,lapse,
                   d_lapse,d_shift,d_spatial_metric,
                   inv_spatial_metric,extrinsic_curvature):
    result = (0.5 * lapse *
