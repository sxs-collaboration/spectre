...
// This checks that choice actually reaches the data manager in the box.
template <typename Generator>
void test_h5_du_dr_j_interpolator_injection(
  ...
  ActionTesting::MockRuntimeSystem<H5InitializeJMetavariables> runner{
      tuples::tagged_tuple_from_typelist<
          ...
          std::make_unique<InitializeJ::CauchySecondOrder>(
              1.0e-11, 1500, true, 1.0e-1, 1.0e-8,
  ...
  const auto& from_box =
      ActionTesting::get_databox_tag<component,
                                     Tags::H5WorldtubeBoundaryDataManager>(
          runner, 0);
  auto substituted = make_manager();
  substituted->set_du_dr_j_interpolator(
      std::make_unique<intrp::BarycentricRationalSpanInterpolator>(
          du_dr_j_order, du_dr_j_order));
  const auto untouched = make_manager();

  const ComplexDataVector box_du_dr_j = boundary_data(from_box);
  const ComplexDataVector substituted_du_dr_j = boundary_data(*substituted);
  const ComplexDataVector untouched_du_dr_j = boundary_data(*untouched);

  // The manager the action left in the box is the one that uses the
  // generator's interpolator ...
  CHECK_ITERABLE_APPROX(box_du_dr_j, substituted_du_dr_j);
  // ... and that is not what `H5Interpolator` alone would have given.
  CHECK(max(abs(box_du_dr_j - untouched_du_dr_j)) > 0.0);
