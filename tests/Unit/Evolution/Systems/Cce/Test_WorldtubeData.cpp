...
void test_bondi_data_manager_du_dy_j() {
  INFO("BondiWorldtubeDataManager::populate_boundary_du_dy_j");
  ...
  // With R constant in time and Dr<BondiJ>(t) = (t / target_time) *
  // Dr<BondiJ>(target_time), the analytic time derivative of
  // dy_j = 0.5 * R * Dr<BondiJ> at the target time is exactly
  // 0.5 * R(target_time) * Dr<BondiJ>(target_time) / target_time
  // at every angular collocation point.
  const auto& bondi_r =
      get(get<Tags::BoundaryValue<Tags::BondiR>>(boundary_variables)).data();
  const auto& dr_bondi_j =
      get(get<Tags::BoundaryValue<Tags::Dr<Tags::BondiJ>>>(boundary_variables))
          .data();
  const ComplexDataVector expected_du_dy_j =
      0.5 * bondi_r * dr_bondi_j / target_time;
  const auto& computed_du_dy_j =
  ...
  Approx interpolator_approx =
      Approx::custom()
          ...
          .scale(1.0);
  CHECK_ITERABLE_CUSTOM_APPROX(computed_du_dy_j, expected_du_dy_j,
                               interpolator_approx);
}
...
void test_data_manager_with_bondi_buffer_updater(
  ...
  // single time, which does not populate `Du<Dy<BondiJ>>` (that boundary value
  // is computed in `BondiWorldtubeDataManager` by time-differentiating the
  // buffered `0.5 * R * Dr<BondiJ>` and so has no single-time analogue here).
...
SPECTRE_TEST_CASE("Unit.Evolution.Systems.Cce.ReadBoundaryDataH5",
  ...
  }
  {
    ...
    test_data_manager_with_bondi_buffer_updater<BondiWorldtubeDataManager,
                                                BondiBufferUpdater>(
        make_not_null(&gen));
    test_bondi_data_manager_du_dy_j();
  }
}
}  // namespace Cce
