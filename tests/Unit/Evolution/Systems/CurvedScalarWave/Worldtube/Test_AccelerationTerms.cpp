  ...
  for (size_t order = 0; order <= 0; ++order) {
    ...
    const auto helper_func =
        [&random_position, &random_velocity, &random_acceleration,
         &random_self_force](const std::array<double, 3>& point) {
          ...
          Worldtube::acceleration_terms_0(
              ...
              random_velocity, random_acceleration, 1.,
              get(random_self_force)[0], get(random_self_force)[1],
              get(random_self_force)[2], get(random_self_force)[3],
              get(random_self_force)[4], get(random_self_force)[5]);
