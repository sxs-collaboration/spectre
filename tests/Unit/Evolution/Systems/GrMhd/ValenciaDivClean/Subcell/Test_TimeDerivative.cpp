...
std::array<double, 4> test(const size_t num_dg_pts) {
  ...
  // const Affine affine_map{-1.0, 1.0, -0.5, 0.5};
  ...
  // We set up a DG element along the +xi direction so from there we have valid
  // mortar data.
  //
  // 0. coords
  // 1. prims
  // 2. cons
  // 3. fluxes
  // 4. normal dot fluxes (divide by -1 / sqrt(gamma^xx))
  const auto dg_face_coords = coordinate_map(interface_logical_coordinates(
      dg_mesh.slice_away(0), Direction<3>::upper_xi()));
