...
def tilde_e_flux(tilde_e, tilde_b, tilde_psi, tilde_phi, tilde_q, tilde_j,
    ...
    result = -np.outer(shift,
                       tilde_e) + lapse * (inv_spatial_metric * tilde_psi)

    for j in range(3):
        for i in range(3):
            for k in range(3):
                result[j, i] -= lapse * levi_civita_symbol(
                    i, j, k) * magnetic_field_one_form[k]
...
def tilde_b_flux(tilde_e, tilde_b, tilde_psi, tilde_phi, tilde_q, tilde_j,
    ...
    result = -np.outer(shift,
                       tilde_b) + lapse * (inv_spatial_metric * tilde_phi)

    for j in range(3):
        for i in range(3):
            for k in range(3):
                result[j, i] += lapse * levi_civita_symbol(
                    i, j, k) * electric_field_one_form[k]
