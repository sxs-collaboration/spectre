...
SPECTRE_TEST_CASE("Unit.Burgers.Fluxes", "[Unit][Burgers]") {
  ...
  const Index<1> extents(num_points);
  const auto coords = get<0>(logical_coordinates(extents));
  ...
  const auto deriv_vars = partial_derivatives<tmpl::list<Burgers::Tags::U>>(
      vars, extents, identity);
  ...
  const auto div_flux = divergence(flux, extents, identity);
