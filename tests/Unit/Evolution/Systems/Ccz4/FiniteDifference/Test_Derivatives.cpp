...
SPECTRE_TEST_CASE("Unit.Evolution.Systems.Ccz4.FiniteDifference.Derivatives",
  ...
  get<::Ccz4::Tags::b<DataVector, 3>>(volume_evolved_vars) =
      get<::Ccz4::Tags::b<DataVector, 3>>(volume_prims_for_recons);
  ...
  auto& expected_d_field_b =
      get<::Tags::deriv<::Ccz4::Tags::b<DataVector, 3>, tmpl::size_t<3>,
                        Frame::Inertial>>(expected_deriv_of_Ccz4_vars);
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j < 3; ++j) {
  ...
  auto& expected_second_d_field_b =
      get<::Tags::second_deriv<::Ccz4::Tags::b<DataVector, 3>, tmpl::size_t<3>,
                               Frame::Inertial>>(
          expected_second_deriv_of_Ccz4_vars);
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j < 3; ++j) {
