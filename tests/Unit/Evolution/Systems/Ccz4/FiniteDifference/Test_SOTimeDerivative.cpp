...
void test_minkowski() {
  ...
  for (auto& component : get<::Tags::dt<gr::Tags::Lapse<DataVector>>>(box)) {
    CHECK_ITERABLE_APPROX(component, zero);
  }
  for (auto& component :
  ...
  for (auto& component :
       get<::Tags::dt<::Ccz4::Tags::b<DataVector, SpatialDim>>>(box)) {
    CHECK_ITERABLE_APPROX(component, zero);
  }
}
...
void test_kerrschild() {
  ...
  // since we assume \partial_\beta = 0.
  ...
  const auto d_b = partial_derivative(
      get<::Ccz4::Tags::b<DataVector, SpatialDim>>(box), subcell_mesh,
      cell_centered_logical_to_inertial_inv_jacobian);
  const tnsr::I<DataVector, SpatialDim, FrameType> dt_b_expected =
      TestHelpers::Ccz4::fd::detail::KerrSchild::get_dt_b_kerr_expected(
          ...
          get<::Ccz4::Tags::b<DataVector, SpatialDim>>(box), d_b);
