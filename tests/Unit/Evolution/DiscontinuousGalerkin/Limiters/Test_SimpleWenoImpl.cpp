...
// IWYU pragma: no_include "Domain/Structure/OrientationMapHelpers.hpp"
...
void test_simple_weno_work(
    ...
    const Mesh<VolumeDim>& mesh, const Element<VolumeDim>& element,
    const std::unordered_map<DirectionId<VolumeDim>,
                             DummyPackagedData<VolumeDim, VectorTag<VolumeDim>>,
                             boost::hash<DirectionId<VolumeDim>>>&
        neighbor_data,
...
void test_simple_weno_1d_impl(const Spectral::Quadrature quadrature,
  ...
  const auto make_neighbor_vars =
      [&logical_coords, &directions_of_external_boundaries, &neighbor_vars,
       &neighbor_modified_vars, &shift_vars_to_local_means](
          const DirectionId<1>& neighbor, const auto make_vars) {
  ...
  make_neighbor_vars(DirectionId<1>{Direction<1>::lower_xi(), ElementId<1>(1)},
                     make_lower_xi_vars);

  make_neighbor_vars(DirectionId<1>{Direction<1>::upper_xi(), ElementId<1>(2)},
                     make_upper_xi_vars);
...
void test_simple_weno_2d_impl(const Spectral::Quadrature quadrature,
  ...
  const auto make_neighbor_vars =
      [&logical_coords, &directions_of_external_boundaries, &neighbor_vars,
       &neighbor_modified_vars, &shift_vars_to_local_means](
          const DirectionId<2>& neighbor, const auto make_vars) {
        if (directions_of_external_boundaries.count(neighbor.direction) == 0) {
          const double offset =
              (neighbor.direction.side() == Side::Lower ? -2.0 : 2.0);
          neighbor_vars[neighbor] = make_vars(logical_coords, offset);
  ...
  make_neighbor_vars(DirectionId<2>{Direction<2>::lower_xi(), ElementId<2>(1)},
                     make_lower_xi_vars);

  make_neighbor_vars(DirectionId<2>{Direction<2>::upper_xi(), ElementId<2>(2)},
                     make_upper_xi_vars);

  make_neighbor_vars(DirectionId<2>{Direction<2>::lower_eta(), ElementId<2>(3)},
                     make_lower_eta_vars);

  make_neighbor_vars(DirectionId<2>{Direction<2>::upper_eta(), ElementId<2>(4)},
                     make_upper_eta_vars);
...
void test_simple_weno_3d_impl(const Spectral::Quadrature quadrature,
  ...
  const auto make_neighbor_vars =
      [&logical_coords, &directions_of_external_boundaries, &neighbor_vars,
       &neighbor_modified_vars, &shift_vars_to_local_means](
          const DirectionId<3>& neighbor, const auto make_vars) {
        if (directions_of_external_boundaries.count(neighbor.direction) == 0) {
          const double offset =
              (neighbor.direction.side() == Side::Lower ? -2.0 : 2.0);
          neighbor_vars[neighbor] = make_vars(logical_coords, offset);
  ...
  make_neighbor_vars(DirectionId<3>{Direction<3>::lower_xi(), ElementId<3>(1)},
                     make_lower_xi_vars);

  make_neighbor_vars(DirectionId<3>{Direction<3>::upper_xi(), ElementId<3>(2)},
                     make_upper_xi_vars);

  make_neighbor_vars(DirectionId<3>{Direction<3>::lower_eta(), ElementId<3>(3)},
                     make_lower_eta_vars);

  make_neighbor_vars(DirectionId<3>{Direction<3>::upper_eta(), ElementId<3>(4)},
                     make_upper_eta_vars);

  make_neighbor_vars(
      DirectionId<3>{Direction<3>::lower_zeta(), ElementId<3>(5)},
      make_lower_zeta_vars);

  make_neighbor_vars(
      DirectionId<3>{Direction<3>::upper_zeta(), ElementId<3>(6)},
