...
void test(const bool use_cell_centered_flux) {
  ...
  if constexpr (Dim > 1) {
    const DirectionId<Dim> south_neighbor_id{Direction<Dim>::lower_eta(),
                                             south_id};
  ...
  for (const auto& [direction, neighbor_ids] : neighbors) {
    ...
    for (const auto& neighbor_id : neighbor_ids) {
      neighbor_decision.insert(
          std::pair{DirectionId<Dim>{direction, neighbor_id}, 0});
    ...
    CHECK_ITERABLE_APPROX(
        ...
        *std::get<2>(
            east_data.at(time_step_id)
                .at(DirectionId<Dim>{Direction<Dim>::lower_xi(), self_id})));
    CHECK(std::get<5>(east_data.at(time_step_id)
                          .at(DirectionId<Dim>{Direction<Dim>::lower_xi(),
