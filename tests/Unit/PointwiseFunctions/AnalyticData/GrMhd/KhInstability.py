...
def rest_mass_density(x, adiabatic_index, strip_bimedian_height,
...
def velocity(x, adiabatic_index, strip_bimedian_height, strip_thickness,
    ...
    result[0] = strip_velocity if (
        np.absolute(x[1] - strip_bimedian_height) < 0.5 *
    ...
    result[1] = (np.exp(-0.5 *
                        ((x[1] - strip_lower_bound) / perturbation_width)**2) +
                 np.exp(-0.5 *
                        ((x[1] - strip_upper_bound) / perturbation_width)**2))
...
def specific_internal_energy(x, adiabatic_index, strip_bimedian_height,
    ...
    return pressure / (adiabatic_index - 1.0) / strip_density if np.absolute(
        x[1] - strip_bimedian_height) < 0.5 * strip_thickness else pressure / (
...
def specific_enthalpy(x, adiabatic_index, strip_bimedian_height,
                      strip_thickness, strip_density, strip_velocity,
                      background_density, background_velocity, pressure,
                      perturbation_amplitude, perturbation_width,
                      magnetic_field):
    return (1.0 + adiabatic_index * specific_internal_energy(
        x, adiabatic_index, strip_bimedian_height, strip_thickness,
        strip_density, strip_velocity, background_density, background_velocity,
        pressure, perturbation_amplitude, perturbation_width, magnetic_field))


def lorentz_factor(x, adiabatic_index, strip_bimedian_height, strip_thickness,
                   strip_density, strip_velocity, background_density,
                   background_velocity, pressure, perturbation_amplitude,
                   perturbation_width, magnetic_field):
    v = velocity(x, adiabatic_index, strip_bimedian_height, strip_thickness,
                 strip_density, strip_velocity, background_density,
                 background_velocity, pressure, perturbation_amplitude,
                 perturbation_width, magnetic_field)
...
def magnetic_field(x, adiabatic_index, strip_bimedian_height, strip_thickness,
                   strip_density, strip_velocity, background_density,
                   background_velocity, pressure, perturbation_amplitude,
                   perturbation_width, magnetic_field):
...
def divergence_cleaning_field(x, adiabatic_index, strip_bimedian_height,
                              strip_thickness, strip_density, strip_velocity,
                              background_density, background_velocity,
