...
void test_compute_phi(const DataType& used_for_size) {
  pypp::check_with_random_values<1>(
      static_cast<tnsr::iaa<DataType, Dim, Frame::Physical> (*)(
          const Scalar<DataType>&,
          const tnsr::i<DataType, Dim, Frame::Physical>&,
          const tnsr::I<DataType, Dim, Frame::Physical>&,
          const tnsr::iJ<DataType, Dim, Frame::Physical>&,
          const tnsr::ii<DataType, Dim, Frame::Physical>&,
          const tnsr::ijj<DataType, Dim, Frame::Physical>&)>(
          &::GeneralizedHarmonic::phi<Dim, Frame::Physical, DataType>),
...
void test_compute_pi(const DataType& used_for_size) {
  pypp::check_with_random_values<1>(
      static_cast<tnsr::aa<DataType, Dim, Frame::Physical> (*)(
          const Scalar<DataType>&, const Scalar<DataType>&,
          const tnsr::I<DataType, Dim, Frame::Physical>&,
          const tnsr::I<DataType, Dim, Frame::Physical>&,
          const tnsr::ii<DataType, Dim, Frame::Physical>&,
          const tnsr::ii<DataType, Dim, Frame::Physical>&,
          const tnsr::iaa<DataType, Dim, Frame::Physical>&)>(
          &::GeneralizedHarmonic::pi<Dim, Frame::Physical, DataType>),
...
void test_compute_gauge_source(const DataType& used_for_size) {
  pypp::check_with_random_values<1>(
      &GeneralizedHarmonic::gauge_source<Dim, Frame::Physical, DataType>,
...
void test_lapse_deriv_functions_analytic(
  ...
  const auto dt_lapse =
      GeneralizedHarmonic::time_deriv_of_lapse<spatial_dim, Frame::Physical,
                                               DataVector>(
          lapse, shift, normal_vector, phi, pi);

  const auto d_lapse =
      GeneralizedHarmonic::spatial_deriv_of_lapse<spatial_dim, Frame::Physical,
                                                  DataVector>(
...
void test_shift_deriv_functions_analytic(
  ...
  auto d4_norm_of_shift_expected =
      make_with_value<tnsr::a<DataVector, SpatialDim, Frame::Physical>>(x, 0.);
  ...
  // Check that locally computed derivs match returned ones
  const auto dt_shift =
      GeneralizedHarmonic::time_deriv_of_shift<SpatialDim, Frame::Physical,
                                               DataVector>(
          lapse, shift, inverse_spatial_metric, normal_vector, phi, pi);

  const auto d_shift =
      GeneralizedHarmonic::spatial_deriv_of_shift<SpatialDim, Frame::Physical,
                                                  DataVector>(
          lapse, inverse_spacetime_metric, normal_vector, phi);

  const auto d4_norm_shift =
      GeneralizedHarmonic::spacetime_deriv_of_norm_of_shift<
          SpatialDim, Frame::Physical, DataVector>(
  ...
  const auto dt_lower_shift = GeneralizedHarmonic::time_deriv_of_lower_shift<
      SpatialDim, Frame::Physical, DataVector>(lapse, shift, spatial_metric,
...
void test_gij_deriv_functions_analytic(
  ...
  const auto dt_gij = GeneralizedHarmonic::time_deriv_of_spatial_metric<
      SpatialDim, Frame::Physical, DataVector>(lapse, shift, phi, pi);
  const auto d_gij =
      GeneralizedHarmonic::deriv_spatial_metric<SpatialDim, Frame::Physical,
                                                DataVector>(phi);
  const auto d4_g = GeneralizedHarmonic::spacetime_deriv_of_det_spatial_metric<
      SpatialDim, Frame::Physical, DataVector>(sqrt_det_spatial_metric,
...
SPECTRE_TEST_CASE("Unit.PointwiseFunctions.GeneralRelativity.GhQuantities",
  ...
  test_lapse_deriv_functions<DataVector, 1, Frame::LastTimeIndependent>(
      used_for_size);
  test_lapse_deriv_functions<DataVector, 2, Frame::LastTimeIndependent>(
      used_for_size);
  test_lapse_deriv_functions<DataVector, 3, Frame::LastTimeIndependent>(
      used_for_size);
  test_lapse_deriv_functions<DataVector, 1, Frame::Physical>(used_for_size);
  test_lapse_deriv_functions<DataVector, 2, Frame::Physical>(used_for_size);
  test_lapse_deriv_functions<DataVector, 3, Frame::Physical>(used_for_size);

  test_gij_deriv_functions<DataVector, 1, Frame::LastTimeIndependent>(
      used_for_size);
  test_gij_deriv_functions<DataVector, 2, Frame::LastTimeIndependent>(
      used_for_size);
  test_gij_deriv_functions<DataVector, 3, Frame::LastTimeIndependent>(
      used_for_size);
  test_gij_deriv_functions<DataVector, 1, Frame::Physical>(used_for_size);
  test_gij_deriv_functions<DataVector, 2, Frame::Physical>(used_for_size);
  test_gij_deriv_functions<DataVector, 3, Frame::Physical>(used_for_size);

  test_shift_deriv_functions<DataVector, 1, Frame::LastTimeIndependent>(
      used_for_size);
  test_shift_deriv_functions<DataVector, 2, Frame::LastTimeIndependent>(
      used_for_size);
  test_shift_deriv_functions<DataVector, 3, Frame::LastTimeIndependent>(
      used_for_size);
  test_shift_deriv_functions<DataVector, 1, Frame::Physical>(used_for_size);
  test_shift_deriv_functions<DataVector, 2, Frame::Physical>(used_for_size);
  test_shift_deriv_functions<DataVector, 3, Frame::Physical>(used_for_size);
  ...
  CHECK(GeneralizedHarmonic::Tags::DerivShiftCompute<3,
                                                     Frame::Physical>::name() ==
        "deriv(Shift)");
  CHECK(GeneralizedHarmonic::Tags::PhiCompute<3, Frame::Physical>::name() ==
        "Phi");
  CHECK(GeneralizedHarmonic::Tags::PiCompute<3, Frame::Physical>::name() ==
        "Pi");
  CHECK(GeneralizedHarmonic::Tags::ExtrinsicCurvatureCompute<
            3, Frame::Physical>::name() == "ExtrinsicCurvature");
  CHECK(GeneralizedHarmonic::Tags::TraceExtrinsicCurvatureCompute<
            3, Frame::Physical>::name() == "TraceExtrinsicCurvature");
  ...
  const auto spacetime_metric = [&]() {
    auto spacetime_metric_l =
        make_with_random_values<tnsr::aa<DataVector, 3, Frame::Physical>>(
  ...
  const auto expected_dt_spatial_metric =
      make_with_random_values<tnsr::ii<DataVector, 3, Frame::Physical>>(
          make_not_null(&generator), make_not_null(&distribution),
          used_for_size);
  const auto expected_dt_shift =
      make_with_random_values<tnsr::I<DataVector, 3, Frame::Physical>>(
          make_not_null(&generator), make_not_null(&distribution),
          used_for_size);
  ...
  const auto expected_deriv_shift =
      make_with_random_values<tnsr::iJ<DataVector, 3, Frame::Physical>>(
          make_not_null(&generator), make_not_null(&distribution),
          used_for_size);
  const auto expected_deriv_spatial_metric =
      make_with_random_values<tnsr::ijj<DataVector, 3, Frame::Physical>>(
          make_not_null(&generator), make_not_null(&distribution),
          used_for_size);
  ...
  const auto box = db::create<
      db::AddSimpleTags<
          ...
          gr::Tags::SpacetimeNormalVector<3, Frame::Physical, DataVector>,
          gr::Tags::InverseSpacetimeMetric<3, Frame::Physical, DataVector>,
          GeneralizedHarmonic::Tags::Phi<3, Frame::Physical>>,
      db::AddComputeTags<
          GeneralizedHarmonic::Tags::DerivSpatialMetricCompute<3,
                                                               Frame::Physical>,
          GeneralizedHarmonic::Tags::DerivLapseCompute<3, Frame::Physical>,
          GeneralizedHarmonic::Tags::DerivShiftCompute<3, Frame::Physical>>>(
  ...
  CHECK(
      db::get<
          ::Tags::deriv<gr::Tags::SpatialMetric<3, Frame::Physical, DataVector>,
                        tmpl::size_t<3>, Frame::Physical>>(box) ==
      expected_deriv_spatial_metric);
  CHECK(db::get<::Tags::deriv<gr::Tags::Lapse<DataVector>, tmpl::size_t<3>,
                              Frame::Physical>>(box) == expected_deriv_lapse);
  CHECK(db::get<::Tags::deriv<gr::Tags::Shift<3, Frame::Physical, DataVector>,
                              tmpl::size_t<3>, Frame::Physical>>(box) ==
  ...
  const auto other_box = db::create<
      ...
      db::AddComputeTags<
          GeneralizedHarmonic::Tags::TimeDerivSpatialMetricCompute<
              3, Frame::Physical>,
          GeneralizedHarmonic::Tags::TimeDerivLapseCompute<3, Frame::Physical>,
  ...
  CHECK(db::get<::Tags::dt<gr::Tags::Lapse<DataVector>>>(other_box) ==
        expected_dt_lapse);
  CHECK(db::get<::Tags::dt<gr::Tags::Shift<3, Frame::Physical, DataVector>>>(
  ...
  CHECK(db::get<GeneralizedHarmonic::Tags::Phi<3, Frame::Physical>>(
            ghvars_box) == expected_phi);
  CHECK(db::get<GeneralizedHarmonic::Tags::Pi<3, Frame::Physical>>(
            ghvars_box) == expected_pi);
  CHECK(db::get<gr::Tags::ExtrinsicCurvature<3, Frame::Physical, DataVector>>(
