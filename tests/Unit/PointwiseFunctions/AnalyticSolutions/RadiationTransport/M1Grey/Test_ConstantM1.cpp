...
struct ConstantM1Proxy : RadiationTransport::M1Grey::Solutions::ConstantM1 {
  ...
  using hydro_variables_tags =
      tmpl::list<hydro::Tags::SpatialVelocity<DataVector, 3, Frame::Physical>,
                 hydro::Tags::LorentzFactor<DataVector>>;

  using m1_variables_tags =
      tmpl::list<RadiationTransport::M1Grey::Tags::TildeE<
                     Frame::Physical, neutrinos::ElectronNeutrinos<0>>,
...
void test_variables(const DataVector& used_for_size) {
  ...
  auto expected_spatial_metric =
      make_with_value<tnsr::ii<DataVector, 3, Frame::Physical>>(used_for_size,
                                                                0.0);
  for (size_t i = 0; i < 3; ++i) {
    expected_spatial_metric.get(i, i) = 1.0;
  }
  const auto spatial_metric = get<
      gr::Tags::SpatialMetric<3, Frame::Physical, DataVector>>(soln.variables(
      coords, 0.0,
      tmpl::list<gr::Tags::SpatialMetric<3, Frame::Physical, DataVector>>{}));
