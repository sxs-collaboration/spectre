...
Variables<valencia_tags> numerical_dt(
    const Solution& solution, const tnsr::I<DataVector, 3, Frame::Physical>& x,
  ...
  using solution_tags =
      tmpl::list<hydro::Tags::RestMassDensity<DataVector>,
                 ...
                 hydro::Tags::MagneticField<DataVector, 3, Frame::Physical>,
...
void verify_grmhd_solution(const Solution& solution,
                           const Block<3, Frame::Physical>& block,
  ...
  const auto& inv_spatial_metric =
      get<gr::Tags::InverseSpatialMetric<3, Frame::Physical, DataVector>>(vars);
  const auto& sqrt_det_spatial_metric =
      get<gr::Tags::SqrtDetSpatialMetric<DataVector>>(vars);
  const auto& extrinsic_curvature =
      get<gr::Tags::ExtrinsicCurvature<3, Frame::Physical, DataVector>>(vars);
  ...
  const auto div_of_fluxes = divergence<flux_tags, 3, Frame::Physical>(
