...
def alfven_specific_internal_energy(
    x,
    t,
    wavenumber,
    pressure,
...
def alfven_divergence_cleaning_field(
    x,
    t,
    wavenumber,
    pressure,
...
def bondi_michel_bernoulli_root_function(
...
def bondi_michel_intermediate_variables(
    ...
    k = bondi_michel_adiabatic_constant(
        n_s_c_2, adiabatic_exponent, sonic_density
    )
    m_dot_over_four_pi = bondi_michel_mass_accretion_rate_over_four_pi(
        sonic_radius, sonic_density, u_c_2
    )
    h_inf_2 = (
        bondi_michel_bernoulli_constant_squared_minus_one(
            n_s_c_2, u_c_2, adiabatic_exponent
        )
        + 1.0
    )
...
def bondi_michel_rest_mass_density(
    x, mass, sonic_radius, sonic_density, adiabatic_exponent, magnetic_field
):
    variables = bondi_michel_intermediate_variables(
        mass, sonic_radius, sonic_density, adiabatic_exponent
    )
...
def bondi_michel_electron_fraction(
    x, mass, sonic_radius, sonic_density, adiabatic_exponent, magnetic_field
):
    return 0.4


def bondi_michel_lorentz_factor(
    x, mass, sonic_radius, sonic_density, adiabatic_exponent, magnetic_field
):
    variables = bondi_michel_intermediate_variables(
        mass, sonic_radius, sonic_density, adiabatic_exponent
    ...
    fluid_speed_u = variables["mass_accretion_rate_over_four_pi"] / (
        r**2
        * bondi_michel_rest_mass_density(
            x,
            mass,
            sonic_radius,
            sonic_density,
    ...
    return bondi_michel_fluid_four_velocity_u_t(
        mass, r, fluid_speed_u
    ) / bondi_michel_one_over_lapse(mass, r)


def bondi_michel_spatial_velocity(
    x, mass, sonic_radius, sonic_density, adiabatic_exponent, magnetic_field
):
    variables = bondi_michel_intermediate_variables(
        mass, sonic_radius, sonic_density, adiabatic_exponent
    ...
    e = (
        ...
        * (
            -fluid_speed_u
            / bondi_michel_fluid_four_velocity_u_t(mass, r, fluid_speed_u)
            + bondi_michel_shift(mass, r)
        )
        / r
    )
...
def bondi_michel_specific_internal_energy(
    x, mass, sonic_radius, sonic_density, adiabatic_exponent, magnetic_field
):
    rest_mass_density = bondi_michel_rest_mass_density(
        x, mass, sonic_radius, sonic_density, adiabatic_exponent, magnetic_field
    )
    pressure = bondi_michel_pressure(
        x, mass, sonic_radius, sonic_density, adiabatic_exponent, magnetic_field
    )
    return pressure / (rest_mass_density * (adiabatic_exponent - 1.0))


def bondi_michel_specific_enthalpy(
    x, mass, sonic_radius, sonic_density, adiabatic_exponent, magnetic_field
):
...
def bondi_michel_pressure(
    ...
    return variables["adiabatic_constant"] * pow(
        rest_mass_density, adiabatic_exponent
    )


def bondi_michel_divergence_cleaning_field(
    x, mass, sonic_radius, sonic_density, adiabatic_exponent, magnetic_field
):
    return 0.0


def bondi_michel_magnetic_field(
    x, mass, sonic_radius, sonic_density, adiabatic_exponent, magnetic_field
):
    r = np.sqrt(x[0] ** 2 + x[1] ** 2 + x[2] ** 2)
