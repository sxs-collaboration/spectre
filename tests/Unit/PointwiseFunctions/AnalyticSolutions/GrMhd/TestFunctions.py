...
def dt_rest_mass_density(x, t, mean_velocity, wave_vector, pressure,
                         adiabatic_exponent, density_amplitude):
    return (-density_amplitude * np.dot(
        np.asarray(wave_vector), np.asarray(mean_velocity)) * np.cos(
            np.dot(
                np.asarray(wave_vector),
                np.asarray(x) - np.asarray(mean_velocity) * t)))
...
def dt_specific_internal_energy(x, t, mean_velocity, wave_vector, pressure,
                                adiabatic_exponent, density_amplitude):
    return (-pressure / ((adiabatic_exponent - 1.0) * rest_mass_density(
        x, t, mean_velocity, wave_vector, pressure,
        adiabatic_exponent, density_amplitude)**2) * dt_rest_mass_density(
            x, t, mean_velocity, wave_vector, pressure, adiabatic_exponent,
            density_amplitude))


def dt_pressure(x, t, mean_velocity, wave_vector, pressure, adiabatic_exponent,
                density_amplitude):
    return 0.0
