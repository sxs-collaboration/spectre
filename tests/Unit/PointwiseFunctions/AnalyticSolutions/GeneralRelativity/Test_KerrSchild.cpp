...
tnsr::I<DataType, 3, Frame::System> spatial_coords(
    const DataType& used_for_size) noexcept {
  auto x =
      make_with_value<tnsr::I<DataType, 3, Frame::System>>(used_for_size, 0.0);
  get<0>(x) = 1.32;
  get<1>(x) = 0.82;
  get<2>(x) = 1.24;
  return x;
...
void test_schwarzschild(const DataType& used_for_size) noexcept {
  ...
  const auto& shift = get<gr::Tags::Shift<3, Frame::System, DataType>>(vars);
  const auto& d_shift =
      get<gr::Solutions::KerrSchild::DerivShift<DataType>>(vars);
  const auto& dt_shift =
      get<Tags::dt<gr::Tags::Shift<3, Frame::System, DataType>>>(vars);
  const auto& g =
      get<gr::Tags::SpatialMetric<3, Frame::System, DataType>>(vars);
  const auto& dt_g =
      get<Tags::dt<gr::Tags::SpatialMetric<3, Frame::System, DataType>>>(vars);
  const auto& d_g =
  ...
  auto expected_d_lapse =
      make_with_value<tnsr::i<DataType, 3, Frame::System>>(x, 0.0);
  for (size_t i = 0; i < 3; ++i) {
  ...
  auto expected_d_shift =
      make_with_value<tnsr::iJ<DataType, 3, Frame::System>>(x, 0.0);
  for (size_t j = 0; j < 3; ++j) {
  ...
  auto expected_g =
      make_with_value<tnsr::ii<DataType, 3, Frame::System>>(x, 0.0);
  for (size_t i = 0; i < 3; ++i) {
  ...
  auto expected_d_g =
      make_with_value<tnsr::ijj<DataType, 3, Frame::System>>(x, 0.0);
