...
void test_generalized_harmonic_solution(const Args&... args) noexcept {
  ...
  const tnsr::I<DataVector, SolutionType::volume_dim, Frame::System> x{
      data_vector};
  ...
  const auto& d_lapse =
      get<Tags::deriv<gr::Tags::Lapse<DataVector>,
                      tmpl::size_t<SolutionType::volume_dim>, Frame::System>>(
          vars);
  const auto& shift =
      get<gr::Tags::Shift<SolutionType::volume_dim, Frame::System, DataVector>>(
          vars);
  const auto& dt_shift = get<Tags::dt<
      gr::Tags::Shift<SolutionType::volume_dim, Frame::System, DataVector>>>(
      vars);
  const auto& d_shift = get<Tags::deriv<
      gr::Tags::Shift<SolutionType::volume_dim, Frame::System, DataVector>,
      tmpl::size_t<SolutionType::volume_dim>, Frame::System>>(vars);
  const auto& g = get<gr::Tags::SpatialMetric<SolutionType::volume_dim,
                                              Frame::System, DataVector>>(vars);
  const auto& dt_g =
      get<Tags::dt<gr::Tags::SpatialMetric<SolutionType::volume_dim,
                                           Frame::System, DataVector>>>(vars);
  const auto& d_g =
      get<Tags::deriv<gr::Tags::SpatialMetric<SolutionType::volume_dim,
                                              Frame::System, DataVector>,
                      tmpl::size_t<SolutionType::volume_dim>, Frame::System>>(
          vars);
  ...
  const auto wrapped_gh_vars = wrapped_solution.variables(
      ...
      tmpl::list<gr::Tags::SpacetimeMetric<SolutionType::volume_dim,
                 ...
                 GeneralizedHarmonic::Tags::Phi<SolutionType::volume_dim,
                                                Frame::System>>{});
  CHECK(psi ==
        get<gr::Tags::SpacetimeMetric<SolutionType::volume_dim, Frame::System,
                                      DataVector>>(wrapped_gh_vars));
  CHECK(pi ==
        get<GeneralizedHarmonic::Tags::Pi<SolutionType::volume_dim,
                                          Frame::System>>(wrapped_gh_vars));
  CHECK(phi ==
