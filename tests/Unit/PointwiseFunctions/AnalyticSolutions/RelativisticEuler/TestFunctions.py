...
def ucase_a(r_sqrd, sin_theta_sqrd, m, a):
    return ((r_sqrd + a**2)**2
            - delta(r_sqrd, m, a) * sin_theta_sqrd * a**2)
...
def kerr_schild_lapse(x, m, a):
    null_vector_0 = -1.0
    return np.sqrt(1.0 / (1.0 + 2.0 * kerr_schild_h(x, m, a) *
                          null_vector_0 * null_vector_0))
...
def angular_velocity(angular_momentum, r_sqrd, sin_theta_sqrd, m, a):
    prefactor = (2.0 * angular_momentum * delta(r_sqrd, m, a) * sin_theta_sqrd /
                 np.power(boyer_lindquist_gff(r_sqrd, sin_theta_sqrd, m, a),
                          2.0))
    return (prefactor / (1.0 + np.sqrt(1.0 + 2.0 * angular_momentum *
                                       prefactor))
...
def u_t(angular_momentum, r_sqrd, sin_theta_sqrd, m, a):
    return np.sqrt(angular_momentum /
                   (boyer_lindquist_gtf(r_sqrd, sin_theta_sqrd, m, a) +
                    angular_velocity(angular_momentum, r_sqrd, sin_theta_sqrd,
                                     m, a) *
                    boyer_lindquist_gff(r_sqrd, sin_theta_sqrd, m, a)))
...
def specific_enthalpy(x, t, black_hole_mass, black_hole_spin, r_in, r_max,
    ...
    if (np.sqrt(r_sqrd * sin_theta_sqrd) >= r_in):
        W = potential(l, r_sqrd, sin_theta_sqrd,
...
def rest_mass_density(x, t, black_hole_mass, black_hole_spin, r_in, r_max,
                      polytropic_constant, polytropic_exponent):
    return np.power((polytropic_exponent - 1.0) *
                    (specific_enthalpy(x, t, black_hole_mass, black_hole_spin,
...
def specific_internal_energy(x, t, black_hole_mass, black_hole_spin, r_in,
    ...
    return (polytropic_constant *
            np.power(rest_mass_density(x, t, black_hole_mass, black_hole_spin,
                                       r_in, r_max, polytropic_constant,
                                       polytropic_exponent),
...
def spatial_velocity(x, t, black_hole_mass, black_hole_spin, r_in,
                     r_max, polytropic_constant, polytropic_exponent):
    l = angular_momentum(black_hole_mass, black_hole_spin, r_max)
    Win = potential(l, r_in * r_in, 1.0, black_hole_mass, black_hole_spin)
    r_sqrd = boyer_lindquist_r_sqrd(x, black_hole_spin)
    ...
    if (np.sqrt(r_sqrd * sin_theta_sqrd) >= r_in):
        W = potential(l, r_sqrd, sin_theta_sqrd,
                      black_hole_mass, black_hole_spin)
        if (W < Win):
            result += ((np.array([-x[1], x[0], 0.0]) *
                        angular_velocity(l, r_sqrd, sin_theta_sqrd,
                                         black_hole_mass, black_hole_spin) +
                        kerr_schild_shift(x, black_hole_mass,
                                          black_hole_spin)) /
                       kerr_schild_lapse(x, black_hole_mass, black_hole_spin))
    return result


def dt_specific_enthalpy(x, t, black_hole_mass, black_hole_spin, r_in, r_max,
                         polytropic_constant, polytropic_exponent):
    return 0.0


def dt_rest_mass_density(x, t, black_hole_mass, black_hole_spin, r_in, r_max,
                         polytropic_constant, polytropic_exponent):
    return 0.0


def dt_specific_internal_energy(x, t, black_hole_mass, black_hole_spin, r_in,
                                r_max, polytropic_constant,
                                polytropic_exponent):
    return 0.0


def dt_pressure(x, t, black_hole_mass, black_hole_spin, r_in, r_max,
                polytropic_constant, polytropic_exponent):
    return 0.0


def dt_spatial_velocity(x, t, black_hole_mass, black_hole_spin, r_in,
                        r_max, polytropic_constant, polytropic_exponent):
