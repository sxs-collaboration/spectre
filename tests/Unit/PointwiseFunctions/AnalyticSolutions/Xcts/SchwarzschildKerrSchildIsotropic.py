...
def shift_magnitude_areal(r, mass):
    return 2. * mass * lapse_areal(r, mass)**2 / r
...
def extrinsic_curvature_areal(x, mass):
    r = np.linalg.norm(x)
    lapse = lapse_areal(r, mass)
    return (2. * mass * lapse / r**2 *
            (np.identity(3) - (2. + mass / r) * np.outer(x, x) / r**2))
...
def isotropic_radius_from_areal(r_areal, mass):
    one_over_lapse = 1. / lapse_areal(r_areal, mass)
...
def areal_radius_from_isotropic(r_isotropic, mass):
    ...
    return newton(func=f, fprime=fprime, x0=r_isotropic, tol=1.e-12)
...
def extrinsic_curvature_trace_gradient(x, mass):
    r_isotropic = np.linalg.norm(x)
    r_areal = areal_radius_from_isotropic(r_isotropic, mass)
    return (extrinsic_curvature_trace_areal_deriv(r_areal, mass) /
            isotropic_radius_from_areal_deriv(r_areal, mass) * x / r_isotropic)
...
def conformal_factor_from_areal_deriv(r_areal, mass):
    one_over_lapse = 1. / lapse_areal(r_areal, mass)
    return (-2. * mass * exp(one_over_lapse - 1.) / (1. + one_over_lapse)**2 /
            r_areal**2)
...
def conformal_factor_gradient(x, mass):
    r_isotropic = np.linalg.norm(x)
    r_areal = areal_radius_from_isotropic(r_isotropic, mass)
    return (conformal_factor_from_areal_deriv(r_areal, mass) /
            isotropic_radius_from_areal_deriv(r_areal, mass) * x / r_isotropic)
...
def lapse_times_conformal_factor_gradient(x, mass):
    r_isotropic = np.linalg.norm(x)
    r_areal = areal_radius_from_isotropic(r_isotropic, mass)
    return ((lapse_areal_deriv(r_areal, mass) * conformal_factor_from_areal(
        r_areal, mass) + lapse_areal(r_areal, mass) *
...
def shift_magnitude_from_areal(r_areal, mass):
    return (shift_magnitude_areal(r_areal, mass) / lapse_areal(r_areal, mass) /
...
def shift_strain(x, mass):
    ...
    return (2. * mass * lapse / r_areal**2 *
            (np.identity(3) + (mass * lapse**3 / r_areal - 2. * lapse) *
             np.outer(x, x) / r_isotropic**2))
