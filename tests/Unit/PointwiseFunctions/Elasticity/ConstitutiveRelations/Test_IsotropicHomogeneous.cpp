...
void test_identity(const tnsr::ii<DataVector, Dim>& random_strain,
                   const tnsr::I<DataVector, Dim>& random_physical_coords) {
  INFO("Identity");
  // This relation should be the negative identity
  const Elasticity::ConstitutiveRelations::IsotropicHomogeneous<Dim> relation{
      1. / 3., 1. / 2.};
  const auto stress = relation.stress(random_strain, random_physical_coords);
...
void test_trace<3>(const tnsr::ii<DataVector, 3>& random_strain,
                   const tnsr::I<DataVector, 3>& random_physical_coords) {
  INFO("Trace");
...
void test_trace<2>(const tnsr::ii<DataVector, 2>& random_strain,
                   const tnsr::I<DataVector, 2>& random_physical_coords) {
  INFO("Trace");
...
void test_analytically() {
  ...
  const auto random_physical_coords =
      make_with_random_values<tnsr::I<DataVector, Dim>>(nn_generator, nn_dist,
                                                        used_for_size);

  test_identity<Dim>(random_strain, random_physical_coords);
  test_trace<Dim>(random_strain, random_physical_coords);
  test_traceless<Dim>(random_strain, random_physical_coords);
}
