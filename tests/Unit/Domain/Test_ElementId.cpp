...
SPECTRE_TEST_CASE("Unit.Domain.ElementId", "[Domain][Unit]") {
  ...
  domain::ElementId<3> test_id;
  CHECK(test_id.block_id() == std::numeric_limits<size_t>::max());
  CHECK(test_id.segment_ids() == make_array<3>(domain::SegmentId()));

  // Test retrieval functions:
  auto segment_ids = std::array<domain::SegmentId, 3>(
      {{domain::SegmentId(2, 3), domain::SegmentId(1, 0),
        domain::SegmentId(1, 1)}});
  domain::ElementId<3> block_2_3d(2, segment_ids);
  CHECK(block_2_3d.block_id() == 2);
  CHECK(block_2_3d.segment_ids() == segment_ids);

  // Test parent and child operations:
  domain::ElementId<3> id = block_2_3d;
  for (size_t dim = 0; dim < 3; dim++) {
    CHECK(id == id.id_of_child(dim, domain::Side::Lower).id_of_parent(dim));
    CHECK(id == id.id_of_child(dim, domain::Side::Upper).id_of_parent(dim));
    if (0 == gsl::at(id.segment_ids(), dim).index() % 2) {
      CHECK(id == id.id_of_parent(dim).id_of_child(dim, domain::Side::Lower));
    } else {
      CHECK(id == id.id_of_parent(dim).id_of_child(dim, domain::Side::Upper));
    }
  }

  // Test equality operator:
  domain::ElementId<3> element_one(1);
  domain::ElementId<3> element_two(1);
  domain::ElementId<3> element_three(2);
  domain::ElementId<3> element_four(4);
...
SPECTRE_TEST_CASE("Unit.Domain.ElementId.ElementIndexConversion",
  ...
  domain::ElementIndex<3> block_2_3d_index(block_2_3d);
  ...
  domain::ElementId<3> block_2_3d_from_index(block_2_3d_index);
