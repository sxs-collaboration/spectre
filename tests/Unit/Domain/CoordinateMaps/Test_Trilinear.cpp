...
SPECTRE_TEST_CASE("Unit.Domain.CoordinateMaps.Trilinear", "[Domain][Unit]") {
  ...
  CHECK((trilinear.inv_jacobian(xi0).get(0, 0)) == inv_jacobian_00);
  CHECK((trilinear.inv_jacobian(xi1).get(0, 0)) == inv_jacobian_00);
  CHECK((trilinear.inv_jacobian(xi2).get(0, 0)) == inv_jacobian_00);

  const double jacobian_00 = 0.5;
  CHECK((trilinear.jacobian(xi0).get(0, 0)) == jacobian_00);
  CHECK((trilinear.jacobian(xi1).get(0, 0)) == jacobian_00);
  CHECK((trilinear.jacobian(xi2).get(0, 0)) == jacobian_00);
