...
namespace domain {
namespace {
void test_coordinate_map(const double xA, const double xB, const double xa,
                         const double xb,
...
SPECTRE_TEST_CASE("Unit.Domain.CoordinateMaps.Interval", "[Domain][Unit]") {
  ...
  CoordinateMaps::Interval logarithmic_map(
      -1.0, 2.0, -3.1, 2.7, CoordinateMaps::Distribution::Logarithmic, -3.2);
  CHECK(get<0>(logarithmic_map(point_1)) == approx(-3.0026932232316066));
  CHECK(get<0>(logarithmic_map(point_2)) == approx(-2.5875856781465209));
  CHECK(get<0>(logarithmic_map(point_3)) == approx(0.80128456770888800));

  test_coordinate_map(-1.0, 2.0, -3.1, 2.7,
                      CoordinateMaps::Distribution::Inverse, -3.2);
  CoordinateMaps::Interval inverse_map(
      -1.0, 2.0, -3.1, 2.7, CoordinateMaps::Distribution::Inverse, -3.2);
  CHECK(get<0>(inverse_map(point_1)) == approx(-3.080405405405405));
  CHECK(get<0>(inverse_map(point_2)) == approx(-3.022408026755852));
  CHECK(get<0>(inverse_map(point_3)) == approx(-2.295620437956204));
