...
SPECTRE_TEST_CASE("Unit.Domain.LogicalCoordinates", "[Domain][Unit]") {
  ...
  const domain::Mesh<1> mesh_1d{3, Spectral::Basis::Legendre,
                                Spectral::Quadrature::GaussLobatto};
  const domain::Mesh<2> mesh_2d{
      {{2, 3}}, Spectral::Basis::Legendre, Spectral::Quadrature::GaussLobatto};

  /// [logical_coordinates_example]
  const domain::Mesh<3> mesh_3d{{{5, 3, 2}},
  ...
  const auto map_3d = domain::make_coordinate_map<Frame::Logical, Frame::Grid>(
      Affine3d{x_map, y_map, z_map});

  const auto x_3d = map_3d(domain::logical_coordinates(mesh_3d));
  /// [logical_coordinates_example]

  const auto map_1d = domain::make_coordinate_map<Frame::Logical, Frame::Grid>(
      domain::CoordinateMaps::Affine{x_map});
  const auto map_2d = domain::make_coordinate_map<Frame::Logical, Frame::Grid>(
      Affine2d{x_map, y_map});
  const auto x_1d = map_1d(domain::logical_coordinates(mesh_1d));
  const auto x_2d = map_2d(domain::logical_coordinates(mesh_2d));
  ...
  CHECK(domain::Tags::LogicalCoordinates<1>::name() == "LogicalCoordinates");
  CHECK(domain::Tags::LogicalCoordinates<2>::name() == "LogicalCoordinates");
  CHECK(domain::Tags::LogicalCoordinates<3>::name() == "LogicalCoordinates");
}
...
SPECTRE_TEST_CASE("Unit.Domain.InterfaceLogicalCoordinates", "[Domain][Unit]") {
  ...
  const domain::Mesh<0> mesh_1d_xbdry;

  const auto x_1d_lb = map_1d(domain::interface_logical_coordinates(
      mesh_1d_xbdry, domain::Direction<1>::lower_xi()));

  CHECK(x_1d_lb[0][0] == -3.0);

  const auto x_1d_ub = map_1d(domain::interface_logical_coordinates(
      mesh_1d_xbdry, domain::Direction<1>::upper_xi()));

  CHECK(x_1d_ub[0][0] == 7.0);

  const domain::Mesh<1> mesh_2d_xbdry{3, Spectral::Basis::Legendre,
                                      Spectral::Quadrature::GaussLobatto};

  const auto x_2d_lb_xi = map_2d(domain::interface_logical_coordinates(
      mesh_2d_xbdry, domain::Direction<2>::lower_xi()));
  ...
  const auto x_2d_ub_xi = map_2d(domain::interface_logical_coordinates(
      mesh_2d_xbdry, domain::Direction<2>::upper_xi()));
  ...
  const domain::Mesh<1> mesh_2d_ybdry{2, Spectral::Basis::Legendre,
                                      Spectral::Quadrature::GaussLobatto};

  const auto x_2d_lb_eta = map_2d(domain::interface_logical_coordinates(
      mesh_2d_ybdry, domain::Direction<2>::lower_eta()));
  ...
  const auto x_2d_ub_eta = map_2d(domain::interface_logical_coordinates(
      mesh_2d_ybdry, domain::Direction<2>::upper_eta()));
  ...
  const domain::Mesh<2> mesh_3d_xbdry{
      {{3, 2}}, Spectral::Basis::Legendre, Spectral::Quadrature::GaussLobatto};

  const auto x_3d_lb_xi = map_3d(domain::interface_logical_coordinates(
      mesh_3d_xbdry, domain::Direction<3>::lower_xi()));
  ...
  const auto x_3d_ub_xi = map_3d(domain::interface_logical_coordinates(
      mesh_3d_xbdry, domain::Direction<3>::upper_xi()));
  ...
  const domain::Mesh<2> mesh_3d_ybdry{
      {{5, 2}}, Spectral::Basis::Legendre, Spectral::Quadrature::GaussLobatto};

  const auto x_3d_lb_eta = map_3d(domain::interface_logical_coordinates(
      mesh_3d_ybdry, domain::Direction<3>::lower_eta()));
  ...
  const auto x_3d_ub_eta = map_3d(domain::interface_logical_coordinates(
      mesh_3d_ybdry, domain::Direction<3>::upper_eta()));
  ...
  const domain::Mesh<2> mesh_3d_zbdry{
      {{5, 3}}, Spectral::Basis::Legendre, Spectral::Quadrature::GaussLobatto};

  const auto x_3d_lb_zeta = map_3d(domain::interface_logical_coordinates(
      mesh_3d_zbdry, domain::Direction<3>::lower_zeta()));
  /// [interface_logical_coordinates_example]
  ...
  const auto x_3d_ub_zeta = map_3d(domain::interface_logical_coordinates(
      mesh_3d_zbdry, domain::Direction<3>::upper_zeta()));
