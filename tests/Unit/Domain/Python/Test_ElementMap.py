...
from spectre.Domain import (ElementMap, deserialize_domain,
...
class TestElementMap(unittest.TestCase):
    def test_element_map(self):
        ...
        domain = Interval(
            ...
            initial_number_of_grid_points_in_x=[3]).create_domain()
        # Element is [0, 1]
        element_id = ElementId[1]("[B0,(L1I0)]")
        element_map = ElementMap(element_id, domain)
        self.assertIsInstance(element_map,
                              CoordinateMapElementLogicalToInertial1D)
        self.assertFalse(element_map.is_identity())
        # Logical coords are [-1, 1]
        xi_data = np.random.rand(1, 4) * 2. - 1.
        xi = tnsr.I[DataVector, 1, Frame.ElementLogical](xi_data)
        npt.assert_allclose(element_map(xi), (np.array(xi) + 1.) / 2.)
        npt.assert_allclose(
            element_map.jacobian(xi).get(0, 0),
            np.ones(4) * 0.5)
        npt.assert_allclose(
            element_map.inv_jacobian(xi).get(0, 0),
            np.ones(4) * 2.)
        # Test inverse
        target_point_inside = tnsr.I[float, 1, Frame.Inertial](fill=0.)
        self.assertAlmostEqual(
            element_map.inverse(target_point_inside).get(0), -1.)
        ...
        self.assertAlmostEqual(
            element_map.inverse(target_point_outside).get(0), 2.)
    ...
    def test_inertial_coordinates(self):
        ...
        inertial_coords = np.asarray(
            element_map(element_logical_coords,
                        time=0.,
