...
SPECTRE_TEST_CASE("Unit.Domain.SegmentId", "[Domain][Unit]") {
  ...
  domain::SegmentId segment_one(4, 3);
  domain::SegmentId segment_two(4, 3);
  domain::SegmentId segment_three(4, 0);
  domain::SegmentId segment_four(5, 4);
  ...
  for (size_t level = 0; level < 5; ++level) {
    ...
    for (size_t segment_index = 0; segment_index < two_to_the(level);
         ++segment_index) {
      domain::SegmentId id(level, segment_index);
      CHECK(id.midpoint() == midpoint);
      CHECK((id.endpoint(domain::Side::Upper) +
             id.endpoint(domain::Side::Lower)) /
                2. ==
            midpoint);
      CHECK(id.endpoint(domain::Side::Upper) -
                id.endpoint(domain::Side::Lower) ==
      ...
      CHECK(id == id.id_of_child(domain::Side::Lower).id_of_parent());
      CHECK(id == id.id_of_child(domain::Side::Upper).id_of_parent());
      CHECK(id.overlaps(id));
      if (0 != level) {
        CHECK(id.overlaps(id.id_of_parent()));
        const domain::Side side_of_parent =
            0 == segment_index % 2 ? domain::Side::Lower : domain::Side::Upper;
      ...
      CHECK(id.id_of_child(domain::Side::Lower).id_of_sibling() ==
            id.id_of_child(domain::Side::Upper));
      CHECK(id.id_of_child(domain::Side::Upper).id_of_sibling() ==
            id.id_of_child(domain::Side::Lower));
      CHECK(
          id.id_of_child(domain::Side::Lower).id_of_abutting_nibling() ==
          id.id_of_child(domain::Side::Upper).id_of_child(domain::Side::Lower));
      CHECK(
          id.id_of_child(domain::Side::Upper).id_of_abutting_nibling() ==
          id.id_of_child(domain::Side::Lower).id_of_child(domain::Side::Upper));
      CHECK(id.id_of_child(domain::Side::Lower).side_of_sibling() ==
            domain::Side::Upper);
      CHECK(id.id_of_child(domain::Side::Upper).side_of_sibling() ==
            domain::Side::Lower);
    }
  }

  // Test retrieval functions:
  domain::SegmentId level_2_index_3(2, 3);
  CHECK(level_2_index_3.refinement_level() == 2);
  CHECK(level_2_index_3.index() == 3);

  // Test output operator:
  domain::SegmentId level_3_index_2(3, 2);
...
[[noreturn]] SPECTRE_TEST_CASE("Unit.Domain.SegmentId.BadIndex",
  ...
  ASSERTION_TEST();
#ifdef SPECTRE_DEBUG
  auto failed_segment_id = domain::SegmentId(3, 8);
...
[[noreturn]] SPECTRE_TEST_CASE("Unit.Domain.SegmentId.NoParent",
  ...
  ASSERTION_TEST();
#ifdef SPECTRE_DEBUG
  auto root_segment_id = domain::SegmentId(0, 0);
