...
void test_compute_horizon_volume_quantities() {
  ...
  const auto& dt_shift =
      get<Tags::dt<gr::Tags::Shift<3, TargetFrame, DataVector>>>(
          solution_vars_target_frame);
  const auto& g = get<gr::Tags::SpatialMetric<3, TargetFrame, DataVector>>(
      solution_vars_target_frame);
  const auto& dt_g =
  ...
  const auto& d_g = get<typename gr::Solutions::KerrSchild::DerivSpatialMetric<
      DataVector, TargetFrame>>(solution_vars_target_frame);
  ...
  if constexpr (std::is_same_v<TargetFrame, Frame::Inertial>) {
    ...
    transform::to_different_frame(make_not_null(&lower_metric_inertial), g,
  ...
  if constexpr (not std::is_same_v<TargetFrame, Frame::Inertial>) {
    ...
    // therefore this test should agree to truncaion error not roundoff.
    ...
    // For a sngle frame there are no numerical derivatives
