...
void test_invert_spec_phys_transform() {
  ...
  const Strahlkorper<Frame::System> sk(l_grid, l_grid, radius, center);

  // Put that Strahlkorper onto a larger grid
  const Strahlkorper<Frame::System> sk_high_res(l_grid_high_res,
                                                l_grid_high_res, sk);
...
void test_copy_and_move() {
  Strahlkorper<Frame::System> s(4, 4, 3.0, {{0.1, 0.2, 0.3}});
...
void test_physical_center() {
  ...
  Strahlkorper<Frame::System> sk(l_max, l_max, radius, expansion_center);
  ...
  // this surface.
  Strahlkorper<Frame::System> sk_test(l_max, l_max, r, expansion_center);
...
void test_point_is_contained() {
  ...
  const Strahlkorper<Frame::System> sphere(3, 2, radius, center);
...
void test_constructor_with_different_coefs(Func function) {
  ...
  Strahlkorper<Frame::System> strahlkorper(4, 4, r, center);
  const Strahlkorper<Frame::System> strahlkorper_test1(4, 4, r + add_to_r,
                                                       center);
...
void test_construct_from_options() {
  Options<tmpl::list<OptionTags::Strahlkorper<Frame::System>>> opts("");
  opts.parse(
      "Strahlkorper:\n"
      " Lmax : 6\n"
      " Center: [1.,2.,3.]\n"
      " Radius: 4.5\n");
  CHECK(opts.get<OptionTags::Strahlkorper<Frame::System>>() ==
        Strahlkorper<Frame::System>(6, 6, 4.5, {{1., 2., 3.}}));
}
...
SPECTRE_TEST_CASE("Unit.ApparentHorizons.Strahlkorper",
  ...
  test_constructor_with_different_coefs(
      [](Strahlkorper<Frame::System>& sk, double add_to_r) noexcept {
        auto coefs = sk.coefficients();  // make a copy
        coefs[0] += sqrt(8.0) * add_to_r;
        return Strahlkorper<Frame::System>(coefs, std::move(sk));
      });
  test_constructor_with_different_coefs(
      [](const Strahlkorper<Frame::System>& sk, double add_to_r) noexcept {
        auto coefs = sk.coefficients();  // make a copy
        coefs[0] += sqrt(8.0) * add_to_r;
        return Strahlkorper<Frame::System>(coefs, sk);
      });
  test_constructor_with_different_coefs(
      [](Strahlkorper<Frame::System>& sk, double add_to_r) noexcept {
        auto& coefs = sk.coefficients();  // no copy
        coefs[0] += sqrt(8.0) * add_to_r;
        return Strahlkorper<Frame::System>(std::move(sk));
      });
...
SPECTRE_TEST_CASE("Unit.ApparentHorizons.Strahlkorper.Serialization",
                  "[ApparentHorizons][Unit]") {
  Strahlkorper<Frame::System> s(4, 4, 2.0, {{1.0, 2.0, 3.0}});
