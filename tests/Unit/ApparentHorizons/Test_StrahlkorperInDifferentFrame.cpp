...
void test_strahlkorper_in_different_frame() {
  ...
  if constexpr (Aligned) {
    strahlkorper_in_different_frame_aligned(
        make_not_null(&strahlkorper_inertial), strahlkorper_grid, domain,
  ...
  // Now compare.
  std::unique_ptr<Strahlkorper<Frame::Inertial>> strahlkorper_expected;
  if constexpr (Aligned) {
    ...
    strahlkorper_expected.reset(new Strahlkorper<Frame::Inertial>(
...
SPECTRE_TEST_CASE("Unit.ApparentHorizons.StrahlkorperInDifferentFrame",
  ...
  test_strahlkorper_in_different_frame<false>();
  test_strahlkorper_in_different_frame<true>();
}
