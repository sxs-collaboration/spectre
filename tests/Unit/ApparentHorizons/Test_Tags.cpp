...
auto create_strahlkorper_y11(const double y11_amplitude, const double radius,
  ...
  Strahlkorper<Frame::Physical> strahlkorper_sphere(l_max, m_max, radius,
                                                    center);
  ...
  return Strahlkorper<Frame::Physical>(coefs, strahlkorper_sphere);
}
...
void test_radius_and_derivs() {
  ...
  db::const_item_type<StrahlkorperTags::DxRadius<Frame::Physical>>
  ...
  const auto& strahlkorper_dx_radius =
      db::get<StrahlkorperTags::DxRadius<Frame::Physical>>(box);
  CHECK_ITERABLE_APPROX(strahlkorper_dx_radius, expected_dx_radius);

  // Test second derivatives.
  db::const_item_type<StrahlkorperTags::D2xRadius<Frame::Physical>>
  ...
  const auto& strahlkorper_laplacian =
      db::get<StrahlkorperTags::LaplacianRadius<Frame::Physical>>(box);
...
void test_normals() {
  ...
  db::const_item_type<StrahlkorperTags::Tangents<Frame::Physical>>
  ...
  db::const_item_type<StrahlkorperTags::CartesianCoords<Frame::Physical>>
  ...
  db::const_item_type<StrahlkorperTags::NormalOneForm<Frame::Physical>>
      expected_normal_one_form(n_pts);
  {
    const auto& r = db::get<StrahlkorperTags::Radius<Frame::Physical>>(box);
  ...
  tnsr::II<DataVector, 3, Frame::Physical> invg(n_pts);
  invg.get(0, 0) = 1.0;
  invg.get(1, 0) = 0.1;
  invg.get(2, 0) = 0.2;
  invg.get(1, 1) = 2.0;
