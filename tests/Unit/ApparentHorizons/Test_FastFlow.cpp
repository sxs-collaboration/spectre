...
FastFlow::Status do_iteration(
    const gsl::not_null<Strahlkorper<Frame::Physical>*> strahlkorper,
  ...
  while (status == FastFlow::Status::SuccessfulIteration) {
    ...
    const auto status_and_info = flow->iterate_horizon_finder<Frame::Physical>(
...
void test_negative_radius_error() {
  ...
  // error exit status.
  Strahlkorper<Frame::Physical> strahlkorper(5, 5, -1.0, {{0, 0, 0}});
...
void test_too_many_iterations_error() {
  Strahlkorper<Frame::Physical> strahlkorper(5, 5, 3.0, {{0, 0, 0}});
...
void test_schwarzschild(FastFlow::Flow::type type_of_flow,
  ...
  const auto iterate_and_check = [&strahlkorper, &flow, &solution ]() noexcept {
    ...
    const auto box = db::create<
        ...
        db::AddComputeTags<
            StrahlkorperTags::compute_items_tags<Frame::Physical>>>(
        strahlkorper);
    const auto& rad = db::get<StrahlkorperTags::Radius<Frame::Physical>>(box);
  ...
  // correctly.
  strahlkorper =
      [](const Strahlkorper<Frame::Physical>& strahlkorper_l) noexcept {
        ...
        return Strahlkorper<Frame::Physical>(coefs, strahlkorper_l);
      }(strahlkorper);
...
void test_kerr(FastFlow::Flow::type type_of_flow, const double mass,
               const size_t max_iterations) {
  Strahlkorper<Frame::Physical> strahlkorper(8, 8, 2.0 * mass, {{0, 0, 0}});
