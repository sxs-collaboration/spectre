...
SPECTRE_TEST_CASE("Unit.ControlSystem.FunctionOfTimeUpdater.Translation",
  ...
  // `system` coordinates. We want to test that the f_of_t (denoted \lambda(t)
  // here) updates properly and continually maps the grid_coords to match the
  // system_coords: grid_coords -> grid_coords + \lambda(t)
  // The target map parameter is
  // \lambda_target(t) = system_coords(t) - grid_coords
  ...
  // Q = system_coords(t) - grid_coords - f_of_t
  // where f_of_t is the current map parameter \lambda(t)
  DataVector grid_coords{{0.2, 0.4}};
  // Here we set up the system coords to have a sinusoidal time
  ...
  DataVector system_coords{grid_coords};
  ...
  while (t < final_time) {
    ...
    trans_error(&updater, f_of_t, t, system_coords);
    // update the FunctionOfTime
    updater.modify(&f_of_t, t);
    // check that Q is within the specified tolerance
    CHECK(fabs(system_coords[0] - grid_coords[0] - f_of_t.func(t)[0][0]) <=
          decrease_timescale_threshold);
    CHECK(fabs(system_coords[1] - grid_coords[1] - f_of_t.func(t)[0][1]) <=
          decrease_timescale_threshold);

    // increase time and get system_coords(t)
    t += dt;
    system_coords[0] = grid_coords[0] + amp1 * sin(omega1 * t);
    system_coords[1] = grid_coords[1] + amp2 * sin(omega2 * t);
  }
}
