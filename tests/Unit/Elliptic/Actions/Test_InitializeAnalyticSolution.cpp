...
struct AnalyticSolution {
  tuples::TaggedTuple<ScalarFieldTag> variables(
      const tnsr::I<DataVector, Dim, Frame::Physical>& x,
  ...
  using phase_dependent_action_list = tmpl::list<
      Parallel::PhaseActions<
          ...
          tmpl::list<ActionTesting::InitializeDataBox<
              tmpl::list<::Tags::Coordinates<Dim, Frame::Physical>>>>>,
...
void test_initialize_analytic_solution(
    const tnsr::I<DataVector, Dim, Frame::Physical>& physical_coords,
  ...
  ActionTesting::emplace_component_and_initialize<element_array>(
      &runner, element_id, {physical_coords});
...
SPECTRE_TEST_CASE("Unit.Elliptic.Actions.InitializeAnalyticSolution",
  ...
  test_initialize_analytic_solution(
      tnsr::I<DataVector, 1, Frame::Physical>{{{{1., 2., 3., 4.}}}},
      Scalar<DataVector>{{{{2., 4., 6., 8.}}}});
  test_initialize_analytic_solution(
      tnsr::I<DataVector, 2, Frame::Physical>{{{{1., 2.}, {3., 4.}}}},
      Scalar<DataVector>{{{{8., 12.}}}});
  test_initialize_analytic_solution(
      tnsr::I<DataVector, 3, Frame::Physical>{{{{1., 2.}, {3., 4.}, {5., 6.}}}},
