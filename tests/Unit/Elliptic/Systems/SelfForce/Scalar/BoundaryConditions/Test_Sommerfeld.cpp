...
SPECTRE_TEST_CASE("Unit.ScalarSelfForce.BoundaryConditions.Sommerfeld",
...

  {
    ...
    const DirectionMap<2, tnsr::I<ComplexDataVector, 2>> alpha_map{
        {direction,
         make_with_value<tnsr::I<ComplexDataVector, 2>>(num_pts, 1.0)}};
    ...
    elliptic::apply_boundary_condition<false, void,
        ...
        boundary_condition, box, direction, make_not_null(&field),
        make_not_null(&n_dot_grad),
...

  {
    ...
    auto field = make_with_value<Scalar<ComplexDataVector>>(num_pts, 1.0);
    auto n_dot_grad = make_with_value<Scalar<ComplexDataVector>>(num_pts, 0.0);
    auto alpha = make_with_value<tnsr::I<ComplexDataVector, 2>>(num_pts, 1.0);
    auto beta = make_with_value<Scalar<ComplexDataVector>>(
        num_pts, std::complex<double>(0.5, 0.0));
    ...
    elliptic::apply_boundary_condition<false, void,
        ...
        make_not_null(&n_dot_grad),
        tnsr::i<ComplexDataVector, 2>{num_pts, 0.0});
    const ComplexDataVector expected(num_pts, std::complex<double>(-0.25, 0.0));
    CHECK_ITERABLE_APPROX(get(n_dot_grad), expected);
  }

  {
    INFO("Apply boundary condition: Order 2, penetrating horizon");
    const auto created = TestHelpers::test_factory_creation<BC, Sommerfeld>(
        "Sommerfeld:\n"
        "  BlackHoleMass: 1.0\n"
        "  BlackHoleSpin: 0.5\n"
        ...
        "  PenetratingHorizon: True\n"
        "  Order: 2");
    const auto& boundary_condition = dynamic_cast<const Sommerfeld&>(*created);
    auto field = make_with_value<Scalar<ComplexDataVector>>(num_pts, 1.0);
    auto n_dot_grad = make_with_value<Scalar<ComplexDataVector>>(num_pts, 0.0);
    // r = 10, M = 1, a = chi*M = 0.5
    // alpha[0] = delta/(r^2+a^2) = (r^2-2Mr+a^2)/(r^2+a^2) = 80.25/100.25
    const double r_test = 10.0;
    const double a = 0.5;
    const double alpha0 =
        (r_test * r_test - 2.0 * r_test + a * a) / (r_test * r_test + a * a);
    auto alpha_ph =
        make_with_value<tnsr::I<ComplexDataVector, 2>>(num_pts, 0.0);
    get<0>(alpha_ph) = ComplexDataVector{num_pts, {alpha0, 0.0}};
    auto beta = make_with_value<Scalar<ComplexDataVector>>(
        num_pts, std::complex<double>(0.5, 0.0));
    auto gamma = make_with_value<tnsr::i<ComplexDataVector, 2>>(num_pts, 0.0);
    get<0>(gamma) = ComplexDataVector{num_pts, {2.0, 0.0}};
    ...
    elliptic::apply_boundary_condition<false, void,
                                       standard_boundary_conditions>(
        boundary_condition, box_ph, direction, make_not_null(&field),
        make_not_null(&n_dot_grad),
        tnsr::i<ComplexDataVector, 2>{num_pts, 0.0});
    // n_dot_flux = -alpha[0] * beta / gamma[0] * field = -alpha0 * 0.25
    const ComplexDataVector expected_ph(
        num_pts, std::complex<double>(-alpha0 * 0.25, 0.0));
    CHECK_ITERABLE_APPROX(get(n_dot_grad), expected_ph);
  }
}
