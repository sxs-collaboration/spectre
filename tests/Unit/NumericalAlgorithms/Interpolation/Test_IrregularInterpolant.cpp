...
const double physical_coord_min = -0.3;
const double physical_coord_max = 0.7;
...
auto make_affine_map<1>() noexcept {
  return domain::make_coordinate_map<Frame::ElementLogical, Frame::Physical>(
...
void test_interpolate_to_points(const Mesh<Dim>& mesh) noexcept {
  ...
  std::uniform_real_distribution<> dist(physical_coord_min, physical_coord_max);

  const auto nn_generator = make_not_null(&generator);
  const auto nn_dist = make_not_null(&dist);

  const size_t number_of_points = 6;
  const auto target_x_physical =
  ...
  const auto target_x = [&target_x_physical, &coordinate_map,
                         &number_of_points]() {
    tnsr::I<DataVector, Dim, Frame::ElementLogical> result(number_of_points);
    for (size_t s = 0; s < number_of_points; ++s) {
      tnsr::I<double, Dim> x_physical_local{};
      for (size_t d = 0; d < Dim; ++d) {
        x_physical_local.get(d) = target_x_physical.get(d)[s];
      }
      const auto x_local = coordinate_map.inverse(x_physical_local).get();
      for (size_t d = 0; d < Dim; ++d) {
  ...
  for (IndexIterator<Dim> iter(mesh.extents()); iter; ++iter) {
    ...
    tmpl::for_each<tags>([&f, &src_x, &target_x_physical, &src_vars,
                          &expected_dest_vars](auto tag) noexcept {
      using Tag = tmpl::type_from<decltype(tag)>;
      get<Tag>(src_vars) = Tag::fill_values(f, src_x);
      get<Tag>(expected_dest_vars) = Tag::fill_values(f, target_x_physical);
    });
