...
void test_definite_integral_1d(const Index<1>& extents) {
  ...
  const DataVector& x = Basis::Legendre::collocation_points(num_pts_in_x);
  ...
  for (size_t a = 0; a < num_pts_in_x; ++a) {
    ...
    if (0 == a % 2) {
      CHECK(2.0 / (a + 1.0) ==
            approx(Basis::Legendre::definite_integral(integrand, extents)));
    } else {
      CHECK(0.0 ==
            approx(Basis::Legendre::definite_integral(integrand, extents)));
    }
...
void test_definite_integral_2d(const Index<2>& extents) {
  ...
  const DataVector& x = Basis::Legendre::collocation_points(num_pts_in_x);
  const DataVector& y = Basis::Legendre::collocation_points(num_pts_in_y);
...
void test_definite_integral_3d(const Index<3>& extents) {
  ...
  const DataVector& x = Basis::Legendre::collocation_points(num_pts_in_x);
  const DataVector& y = Basis::Legendre::collocation_points(num_pts_in_y);
  const DataVector& z = Basis::Legendre::collocation_points(num_pts_in_z);
  DataVector integrand(extents.product());
  for (size_t a = 0; a < num_pts_in_x; ++a) {
    for (size_t b = 0; b < num_pts_in_y; ++b) {
...
SPECTRE_TEST_CASE("Unit.Numerical.LinearOperators.DefiniteIntegral",
  ...
  for (size_t n0 = min_extents; n0 <= Basis::Legendre::maximum_number_of_pts;
       ++n0) {
  ...
  for (size_t n0 = min_extents; n0 <= Basis::Legendre::maximum_number_of_pts;
    ...
    for (size_t n1 = min_extents;
         n1 <= Basis::Legendre::maximum_number_of_pts - 1; ++n1) {
      test_definite_integral_2d(Index<2>(n0, n1));
    }
  }
  for (size_t n0 = min_extents;
       n0 <= std::min(6_st, Basis::Legendre::maximum_number_of_pts); ++n0) {
    for (size_t n1 = min_extents;
         n1 <= std::min(7_st, Basis::Legendre::maximum_number_of_pts); ++n1) {
      for (size_t n2 = min_extents;
           n2 <= std::min(8_st, Basis::Legendre::maximum_number_of_pts); ++n2) {
