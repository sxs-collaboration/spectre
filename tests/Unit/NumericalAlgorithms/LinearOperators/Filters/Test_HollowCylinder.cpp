...
using CylinderFilter = Filters::HollowCylinder<TagList>;
...
// Construct the canonical hollow-cylinder shell mesh:
//   dim 0 (radial) Legendre, dim 1 (angular) Fourier, dim 2 (z) Legendre.
Mesh<3> shell_mesh() {
  return Mesh<3>{
...
// Independent reference for the volume filter: applies the radial, angular
// exponential, angular top-mode cutoff, and axial filters sequentially.
Variables<TagList> expected_volume(const Mesh<3>& mesh,
                                   const Variables<TagList>& initial,
                                   const size_t num_modes_to_kill,
                                   const std::optional<size_t> angular_half,
                                   const std::optional<size_t> radial_half,
                                   const std::optional<size_t> z_half) {
  auto v = initial;
  ...
  if (angular_half.has_value()) {
    v = apply_one(
        mesh, v, 1,
        Spectral::filtering::exponential_filter(
            mesh.slice_through(1), 36.0, static_cast<unsigned>(*angular_half)));
  }
  if (num_modes_to_kill > 0) {
    v = apply_one(mesh, v, 1,
                  Spectral::filtering::zero_highest_modes(mesh.slice_through(1),
                                                          num_modes_to_kill));
  }
  if (z_half.has_value()) {
    v = apply_one(
        mesh, v, 2,
        Spectral::filtering::exponential_filter(
            mesh.slice_through(2), 36.0, static_cast<unsigned>(*z_half)));
  }
  return v;
}
...
void test_construction_and_accessors() {
  ...
  const auto filter = CylinderFilter(1, 4, 4, 4, true, blocks_opt, false, true,
                                     std::nullopt, 3);

  CHECK_FALSE(filter.need_jacobians());
  CHECK(filter.name() == "HollowCylinder");
  ...
  const auto unrestricted =
      CylinderFilter(0, std::nullopt, 8, std::nullopt, true, std::nullopt, true,
                     false, 1, std::nullopt);
  ...
  const auto disabled =
      CylinderFilter(1, 4, 4, 4, false, std::nullopt, true, true, 1, 1);
  ...
  const auto base =
      CylinderFilter(1, 4, 4, 4, true, blocks_opt, false, false, 2, 5);
  CHECK(base ==
        CylinderFilter(1, 4, 4, 4, true, blocks_opt, false, false, 2, 5));
  CHECK_FALSE(base !=
              CylinderFilter(1, 4, 4, 4, true, blocks_opt, false, false, 2, 5));
  CHECK(base !=
        CylinderFilter(2, 4, 4, 4, true, blocks_opt, false, false, 2, 5));
  CHECK(base != CylinderFilter(1, std::nullopt, 4, 4, true, blocks_opt, false,
                               false, 2, 5));
  CHECK(base != CylinderFilter(1, 4, std::nullopt, 4, true, blocks_opt, false,
                               false, 2, 5));
  CHECK(base != CylinderFilter(1, 4, 4, std::nullopt, true, blocks_opt, false,
                               false, 2, 5));
  CHECK(base !=
        CylinderFilter(1, 4, 4, 4, false, blocks_opt, false, false, 2, 5));
  CHECK(base !=
        CylinderFilter(1, 4, 4, 4, true, std::nullopt, false, false, 2, 5));
  CHECK(base !=
        CylinderFilter(1, 4, 4, 4, true, blocks_opt, true, false, 2, 5));
  CHECK(base !=
        CylinderFilter(1, 4, 4, 4, true, blocks_opt, false, true, 2, 5));
  CHECK(base != CylinderFilter(1, 4, 4, 4, true, blocks_opt, false, false,
                               std::nullopt, 5));
  CHECK(base != CylinderFilter(1, 4, 4, 4, true, blocks_opt, false, false, 2,
                               std::nullopt));

  CHECK_THROWS_WITH((CylinderFilter{1, 4, 4, 4, true,
...
void test_is_equal() {
  ...
  const std::optional<std::vector<std::string>> blocks{
      std::vector<std::string>{"Block0", "Group1"}};
  const auto a =
      CylinderFilter(1, 4, 4, 4, true, blocks, false, false, 2, std::nullopt);
  const auto b =
      CylinderFilter(1, 4, 4, 4, true, blocks, false, false, 2, std::nullopt);
  const auto c =
      CylinderFilter(2, 4, 4, 4, true, blocks, false, false, 2, std::nullopt);
  CHECK(a.is_equal(b));
...
void test_supports_mesh() {
  INFO("supports_mesh");
  const auto filter = CylinderFilter(1, 4, 4, 4, true, std::nullopt, false,
                                     false, std::nullopt, std::nullopt);

  // The canonical hollow-cylinder shell mesh is supported, including the
  // Chebyshev variant in the radial/axial directions.
  CHECK(filter.supports_mesh(shell_mesh()));
  CHECK(filter.supports_mesh(Mesh<3>{
      std::array<size_t, 3>{5, 7, 4},
      std::array<Spectral::Basis, 3>{Spectral::Basis::Chebyshev,
                                     Spectral::Basis::Fourier,
  ...
  // A filled-cylinder (ZernikeB2) mesh is not a hollow cylinder.
  CHECK_FALSE(filter.supports_mesh(
      Mesh<3>{std::array<size_t, 3>{5, 5, 4},
  ...
  // No Fourier direction.
  CHECK_FALSE(filter.supports_mesh(
      Mesh<3>{5, Spectral::Basis::Legendre, Spectral::Quadrature::Gauss}));
  // Fourier in the wrong (radial) direction.
  CHECK_FALSE(filter.supports_mesh(Mesh<3>{
      std::array<size_t, 3>{7, 5, 4},
      std::array<Spectral::Basis, 3>{Spectral::Basis::Fourier,
                                     Spectral::Basis::Legendre,
                                     Spectral::Basis::Legendre},
      std::array<Spectral::Quadrature, 3>{
          Spectral::Quadrature::Equiangular, Spectral::Quadrature::Gauss,
          Spectral::Quadrature::GaussLobatto}}));
...
void test_apply_in_volume() {
  ...
  const Mesh<3> mesh = shell_mesh();
  const auto initial_vars = deterministic_vars<3>(mesh);

  struct Params {
    size_t num_modes_to_kill;
    std::optional<size_t> angular_half;
    std::optional<size_t> radial_half;
    std::optional<size_t> z_half;
  };
  const std::vector<Params> cases{
      {1, 4, 4, 4},                                   // everything active
      {2, std::nullopt, std::nullopt, std::nullopt},  // only the angular cutoff
      {0, std::nullopt, 4, std::nullopt},             // only radial
      {0, 4, std::nullopt, 6},                        // angular exp + axial
      {0, std::nullopt, std::nullopt, std::nullopt}};  // identity

  for (const auto& p : cases) {
    CAPTURE(p.num_modes_to_kill);
    const auto filter = CylinderFilter(
        p.num_modes_to_kill, p.angular_half, p.radial_half, p.z_half, true,
    ...
    filter.apply_in_volume(make_not_null(&vars), mesh, std::nullopt,
                           std::nullopt);
    const auto expected =
        expected_volume(mesh, initial_vars, p.num_modes_to_kill, p.angular_half,
                        p.radial_half, p.z_half);
  ...
  const auto cutoff_filter =
      CylinderFilter(2, std::nullopt, std::nullopt, std::nullopt, true,
                     std::nullopt, false, false, std::nullopt, std::nullopt);
...
void test_apply_on_boundary() {
  ...
  const Approx custom_approx = Approx::custom().epsilon(5.0e-13);
  const Mesh<3> volume_mesh = shell_mesh();
  const size_t num_modes_to_kill = 1;
  const std::optional<size_t> angular_half = 4;
  const std::optional<size_t> radial_half = 4;
  const std::optional<size_t> z_half = 5;
  const auto filter =
      CylinderFilter(num_modes_to_kill, angular_half, radial_half, z_half, true,
                     std::nullopt, false, false, std::nullopt, std::nullopt);

  // Radial face: slice away dim 0 -> face dims (Fourier=angular, Legendre=z).
  {
    ...
    expected = apply_one(
        face, expected, 0,
        Spectral::filtering::exponential_filter(
            face.slice_through(0), 36.0, static_cast<unsigned>(*angular_half)));
    expected = apply_one(face, expected, 0,
                         Spectral::filtering::zero_highest_modes(
                             face.slice_through(0), num_modes_to_kill));
    expected = apply_one(
        face, expected, 1,
        Spectral::filtering::exponential_filter(
  ...
  // Angular faces do not exist: the angular (Fourier) direction is periodic, so
  // slicing it away to form a (Legendre=radial, Legendre=z) boundary face must
  // error.
  {
  ...
  // Axial face: slice away dim 2 -> face dims (Legendre=radial,
  // Fourier=angular)
  {
    ...
    expected = apply_one(
        face, expected, 0,
        Spectral::filtering::exponential_filter(
            face.slice_through(0), 36.0, static_cast<unsigned>(*radial_half)));
    expected = apply_one(
        face, expected, 1,
        Spectral::filtering::exponential_filter(
            face.slice_through(1), 36.0, static_cast<unsigned>(*angular_half)));
    expected = apply_one(face, expected, 1,
                         Spectral::filtering::zero_highest_modes(
                             face.slice_through(1), num_modes_to_kill));
    CHECK_VARIABLES_CUSTOM_APPROX(vars, expected, custom_approx);
  }
}
...
void test_option_parsing() {
  ...
  auto invalid_filter =
      CylinderFilter(0, std::nullopt, 4, std::nullopt, true,
...
void test_errors() {
  INFO("Errors");
#ifdef SPECTRE_DEBUG
  // Zeroing more angular m-modes than the Fourier basis resolves trips the
  // zero_highest_modes assertion at apply time: the angular extent of
  // shell_mesh() is 7 (3 m-modes), so killing 4 is invalid. HollowCylinder
  // does no construction-time validation of NumModesToKill against any extent.
  const Mesh<3> mesh = shell_mesh();
  const auto filter =
      CylinderFilter(4, std::nullopt, std::nullopt, std::nullopt, true,
                     std::nullopt, false, false, std::nullopt, std::nullopt);
...
SPECTRE_TEST_CASE("Unit.Numerical.LinearOperators.Filter.HollowCylinder",
