...
def wcns5z(q):
    def wcns5z_oneside(q, s):
        ...
        b0 = 0.25 * (q[j - 2 * s] - 4.0 * q[j - s] + 3.0 * q[j])**2 + (
            13.0 / 12.0) * (q[j - 2 * s] - 2.0 * q[j - s] + q[j])**2
        b1 = 0.25 * (q[j + s] - q[j - s])**2 + (13.0 / 12.0) * (
            q[j - s] + q[j + s] - 2.0 * q[j])**2
        b2 = 0.25 * (q[j + 2 * s] - 4.0 * q[j + s] + 3.0 * q[j])**2 + (
            13.0 / 12.0) * (q[j + 2 * s] - 2.0 * q[j + s] + q[j])**2
        ...
        alpha = np.asarray([1, 10, 5]) / 16.0 * (1.0 + (tau5 /
                                                        (beta + epsilon))**2)
        nw = alpha / np.sum(alpha)

        recons_stencils = np.asarray([
...
def test_wcns5z_with_minmod(u, extents, dim):
    ...
    def compute_face_values(recons_upper_of_cell, recons_lower_of_cell, v, i,
        ...
        return compute_face_values_t(recons_upper_of_cell,
                                     recons_lower_of_cell, v, i, j, k,
                                     dim_to_recons, wcns5z_with_minmod)
