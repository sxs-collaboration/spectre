...
#include "ParallelAlgorithms/Interpolation/LagrangePolynomial.hpp"
...
SPECTRE_TEST_CASE("Unit.Numerical.Interpolation.LagrangePolynomial",
  ...
  for (size_t i = 0; i < control.size(); ++i) {
    for (size_t j = 0; j < control.size(); ++j) {
      CHECK(lagrange_polynomial(j, gsl::at(control, i), control.begin(),
                                control.end()) == approx(i == j ? 1. : 0.));
      CHECK(lagrange_polynomial(control.begin() + static_cast<ptrdiff_t>(j),
                                gsl::at(control, i), control.begin(),
                                control.end()) == approx(i == j ? 1. : 0.));
    }
  }
  ...
  for (auto it = control.begin(); it != control.end(); ++it) {
    interpolated +=
        func(*it) *
        lagrange_polynomial(it, test_point, control.begin(), control.end());
  }
