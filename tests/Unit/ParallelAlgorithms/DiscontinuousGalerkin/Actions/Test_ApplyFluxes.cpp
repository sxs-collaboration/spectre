...
#include "ParallelAlgorithms/DiscontinuousGalerkin/SimpleBoundaryData.hpp"
...
SPECTRE_TEST_CASE("Unit.DG.Actions.ApplyFluxes",
  ...
  const auto make_mortar_data = [](
      const Scalar<DataVector>& local_var, const Scalar<DataVector>& remote_var,
  ...
  const DataVector xi_flux = {0., 0., 7011., 0., 0., 8023., 0., 0., 9035.};
  const DataVector eta_flux = {10042., 11054., 12066., 0., 0., 0., 0., 0., 0.};
...
SPECTRE_TEST_CASE("Unit.DG.Actions.ApplyFluxes.h-refinement",
                  "[Unit][NumericalAlgorithms][Actions]") {
