...
// IWYU pragma: no_include "ParallelAlgorithms/DiscontinuousGalerkin/SimpleBoundaryData.hpp"
...
void run_lts_case(const int self_step_end, const std::vector<int>& left_steps,
  ...
  const auto& recorders =
      ActionTesting::get_databox_tag<my_component, MortarRecorderTag>(runner,
                                                                      self_id);
  const auto check_data =
      [](const auto& recorder, const std::vector<int>& steps) noexcept {
