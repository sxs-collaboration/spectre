...
SPECTRE_TEST_CASE("Unit.ApparentHorizonFinder.OptionTags",
...

  {
    const auto blocks_for_interpolation =
        ah::Tags::BlocksForInterpolation2::create_from_options<
            MockMetavariables>(
            std::make_unique<domain::creators::Sphere>(
                1.8, 2.2, domain::creators::Sphere::Excision{}, 1_st, 5_st,
                false),
            created_opts);
    REQUIRE(blocks_for_interpolation.contains("MockHorizonMetavars"));
    const auto block_names = domain_creator.block_names();
    CHECK(blocks_for_interpolation.at("MockHorizonMetavars") ==
