...
MockReceiveVolumeData::Results MockReceiveVolumeData::results{};
...
struct MockComponent {
  ...
  using const_global_cache_tags =
      tmpl::list<domain::Tags::Domain<3>, ah::Tags::BlocksForInterpolation2>;
  ...
  using replace_these_simple_actions =
      tmpl::list<ah::ReceiveVolumeData<MockHorizonMetavars>>;
  using with_these_simple_actions = tmpl::list<MockReceiveVolumeData>;
};
...
SPECTRE_TEST_CASE("Unit.ApparentHorizonFinder.FindApparentHorizonEvent",
  ...
  const auto check_results = [&]() {
    ...
    const auto& results = MockReceiveVolumeData::results;
