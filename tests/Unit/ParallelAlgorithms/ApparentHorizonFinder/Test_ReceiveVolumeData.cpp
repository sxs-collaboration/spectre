...
struct MockComponent {
  ...
  using phase_dependent_action_list = tmpl::list<Parallel::PhaseActions<
      ...
      tmpl::list<Initialization::Actions::InitializeItems<
          ah::Initialize<Metavariables, HorizonMetavars>>>>>;
};
  ...
  for (const auto& time : times) {
    for (const auto& element_id : element_ids) {
      ...
      ActionTesting::queue_simple_action<
          component, ah::ReceiveVolumeData<horizon_metavars>>(
...
// [[TimeOut, 60]]
SPECTRE_TEST_CASE("Unit.ApparentHorizonFinder.ReceiveVolumeData",
