...
// TODO: Ask Mark if there's an easy way to check the spatial RicciScalar on a
...
Variables<ah::vars_to_interpolate_to_target<3, Fr>> make_vars(
  ...
  get<0, 0>(get<gr::Tags::SpatialRicci<DataVector, 3, Fr>>(result)) =
      -8.0 * mass * radius / square(2.0 * square(radius) + mass * radius);
  get<1, 1>(get<gr::Tags::SpatialRicci<DataVector, 3, Fr>>(result)) =
      4.0 * cube(radius) * mass / square(2.0 * square(radius) + mass * radius);
  const auto theta_phis = horizon.ylm_spherepack().theta_phi_points();
  get<2, 2>(get<gr::Tags::SpatialRicci<DataVector, 3, Fr>>(result)) =
      square(sin(theta_phis[0])) *
      get<1, 1>(get<gr::Tags::SpatialRicci<DataVector, 3, Fr>>(result));
...
void check_ylm_data(
  ...
  // Center is origin
  std::vector<double> ylm_expected_data{2.0, 0.0, 0.0, 0.0, l_max};
...
void check_surface_volume_data(
    const std::string& surfaces_file_prefix,
    const ylm::Strahlkorper<Frame::Inertial>& strahlkorper,
    const LinkedMessageId<double>& time, const double radius,
    const double mass) {
  ...
  const auto all_vars = make_vars(strahlkorper, time, radius, mass);
  const auto& ricci_tensor =
      get<gr::Tags::SpatialRicci<DataVector, 3>>(all_vars);
  const auto& inverse_spatial_metric =
      get<gr::Tags::InverseSpatialMetric<DataVector, 3>>(all_vars);
  const auto ricci_scalar = tenex::evaluate(
      inverse_spatial_metric(ti::I, ti::J) * ricci_tensor(ti::i, ti::j));
  const std::vector<DataVector> tensor_and_coord_data{
      get<0>(coords), get<1>(coords), get<2>(coords), get(ricci_scalar)};
...
struct MockComponent {
  ...
  using phase_dependent_action_list = tmpl::list<Parallel::PhaseActions<
      ...
      tmpl::list<ActionTesting::InitializeDataBox<
          typename ah::Initialize<Metavariables, HorizonMetavars>::simple_tags,
          typename ah::Initialize<Metavariables,
                                  HorizonMetavars>::compute_tags>>>>;
};
...
void run_test() {
  ...
  const auto initialize_component = [&](auto component_v) {
    using component = std::decay_t<decltype(component_v)>;
    using Fr = typename component::frame;

    const ylm::Strahlkorper<Fr> horizon{l_max, radius,
                                        std::array{0.0, 0.0, 0.0}};
    ...
    db::mutate<ylm::Tags::Strahlkorper<Fr>, ah::Tags::CurrentTime,
        ...
        [&](const gsl::not_null<ylm::Strahlkorper<Fr>*> strahlkorper,
          ...
          (*current_time) = time;
          (*vars) = make_vars(horizon, time, radius, mass);
        },
        make_not_null(&box));
  };

  initialize_component(horizon_a_component{});
  initialize_component(horizon_b_component{});
  initialize_component(horizon_c_component{});
  initialize_component(horizon_d_component{});

  // const std::array spin{1.0, 0.0, 0.0};
  // const ylm::Strahlkorper<Frame::Inertial> horizon_d(
  //     l_max, l_max,
  //     get(gr::Solutions::kerr_horizon_radius(
  //         ::ylm::Spherepack(l_max, l_max).theta_phi_points(), mass, spin)),
  //     center);
  const ylm::Strahlkorper<Frame::Inertial> horizon_d{l_max, radius,
                                                     std::array{0.0, 0.0, 0.0}};
  // {
  //   ActionTesting::emplace_array_component<horizon_d_component>(
  //       make_not_null(&runner), ActionTesting::NodeId{0},
  //       ActionTesting::LocalCoreId{0}, 0);

  //   auto& box = ActionTesting::get_databox<horizon_d_component>(
  //       make_not_null(&runner), 0);

  //   db::mutate<ylm::Tags::Strahlkorper<::Frame::Inertial>,
  //              ah::Tags::CurrentTime,
  //              ::Tags::Variables<
  //                  ah::vars_to_interpolate_to_target<3, ::Frame::Inertial>>>(
  //       [&](const gsl::not_null<ylm::Strahlkorper<::Frame::Inertial>*>
  //               strahlkorper,
  //           const gsl::not_null<std::optional<LinkedMessageId<double>>*>
  //               current_time,
  ...
  //               ah::vars_to_interpolate_to_target<3, ::Frame::Inertial>>*>
  //               vars) {
  //         (*strahlkorper) = horizon_d;
  //         (*current_time) = time;
  //         (*vars) = make_vars(horizon_d, time, radius, mass);
  //       },
  //       make_not_null(&box));
  // }
  ...
  // TODO: Figure out why??
  const std::vector<double> expected_integral_c{};
  ...
  check_ylm_data(h5_file_name, horizon_d);
  ...
  check_surface_volume_data(surfaces_file_prefix, horizon_d, time, radius,
                            mass);
