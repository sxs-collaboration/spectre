...
// [[OutputRegex, Each member of the vector<vector<double>> must be of the same
// size, ie the number of columns must be the same.]]
SPECTRE_TEST_CASE("Unit.IO.H5.DatErrorAppendVectorVector", "[Unit][IO][H5]") {
  ERROR_TEST();
  // We expect this test to segfault, however the error message is still useful,
  // so we pretend all is fine so that the test can pass.
  std::signal(SIGSEGV, [](int) { std::exit(0); });
  const std::string h5_file_name("Unit.IO.H5.DatErrorAppendVectorVector.h5");
  const uint32_t version_number = 4;
  if (file_system::check_if_file_exists(h5_file_name)) {
    file_system::rm(h5_file_name, true);
  }
  ...
  error_file.append(std::vector<std::vector<double>>{
      {0.00, 0.10, 0.20, 0.30}, {0.11, 0.40, 0.50}, {0.22, 0.55, 0.60, 0.80}});
}

// [[OutputRegex, Cannot add columns to Dat files. Current number of columns is
// 4 but received 3 entries.]]
SPECTRE_TEST_CASE("Unit.IO.H5.DatErrorAppendVectorVectorColumns",
                  "[Unit][IO][H5]") {
  ERROR_TEST();
  // We expect this test to segfault, however the error message is still useful,
  // so we pretend all is fine so that the test can pass.
  ...
  const std::string h5_file_name(
      "Unit.IO.H5.DatErrorAppendVectorVectorColumns.h5");
  const uint32_t version_number = 4;
  if (file_system::check_if_file_exists(h5_file_name)) {
    file_system::rm(h5_file_name, true);
  }
  ...
  error_file.append(std::vector<std::vector<double>>{
      {0.00, 0.10, 0.20}, {0.11, 0.40, 0.50, 0.60}, {0.22, 0.55, 0.60, 0.80}});
}

// [[OutputRegex, Cannot add columns to Dat files. Current number of columns is
// 4 but received 3 entries.]]
SPECTRE_TEST_CASE("Unit.IO.H5.DatErrorAppendMatrixColumns", "[Unit][IO][H5]") {
  ERROR_TEST();
  // We expect this test to segfault, however the error message is still useful,
  // so we pretend all is fine so that the test can pass.
  std::signal(SIGSEGV, [](int) { std::exit(0); });
  const std::string h5_file_name("Unit.IO.H5.DatErrorAppendMatrixColumns.h5");
  const uint32_t version_number = 4;
  if (file_system::check_if_file_exists(h5_file_name)) {
    file_system::rm(h5_file_name, true);
  }
...
// [[OutputRegex, Cannot add columns to Dat files. Current number of columns is
// 4 but received 3 entries.]]
SPECTRE_TEST_CASE("Unit.IO.H5.DatErrorAppendVectorColumns", "[Unit][IO][H5]") {
  ERROR_TEST();
  // We expect this test to segfault, however the error message is still useful,
  // so we pretend all is fine so that the test can pass.
  std::signal(SIGSEGV, [](int) { std::exit(0); });
  const std::string h5_file_name("Unit.IO.H5.DatErrorAppendVectorColumns.h5");
  const uint32_t version_number = 4;
  if (file_system::check_if_file_exists(h5_file_name)) {
    file_system::rm(h5_file_name, true);
  }
  ...
  error_file.append(std::vector<double>{0.00, 0.10, 0.20});
}

// [[OutputRegex, Mismatch in the size of the read dataset. Read \(6,4\) but
// have stored \(3,4\). This means that another thread or process is writing
// data at the same time it is being read.]]
SPECTRE_TEST_CASE("Unit.IO.H5.DatErrorGetDataSizeChange", "[Unit][IO][H5]") {
  ERROR_TEST();
  // We expect this test to segfault, however the error message is still useful,
  // so we pretend all is fine so that the test can pass.
  std::signal(SIGSEGV, [](int) { std::exit(0); });
  const std::string h5_file_name("Unit.IO.H5.DatErrorGetDataSizeChange.h5");
  const uint32_t version_number = 4;
  if (file_system::check_if_file_exists(h5_file_name)) {
    file_system::rm(h5_file_name, true);
  }
  ...
  h5::H5File<h5::AccessType::ReadWrite> my_file2(h5_file_name, true);
  auto& error_file2 =
      my_file2.get<h5::Dat>("/L2_errors", legend, version_number);
  error_file2.append(
      std::vector<std::vector<double>>{{0.00, 0.10, 0.20, 0.30},
                                       {0.11, 0.40, 0.50, 0.60},
                                       {0.22, 0.55, 0.60, 0.80}});
  const auto data_read = error_file.get_data();
}

// [[OutputRegex, Mismatch in the size of the read dataset. Read \(6,4\) but
// have stored \(3,4\). This means that another thread or process is writing
// data at the same time it is being read.]]
SPECTRE_TEST_CASE("Unit.IO.H5.DatErrorGetDataSubsetSizeChange",
                  "[Unit][IO][H5]") {
  ERROR_TEST();
  // We expect this test to segfault, however the error message is still useful,
  // so we pretend all is fine so that the test can pass.
  std::signal(SIGSEGV, [](int) { std::exit(0); });
  const std::string h5_file_name(
      "Unit.IO.H5.DatErrorGetDataSubsetSizeChange.h5");
  const uint32_t version_number = 4;
  if (file_system::check_if_file_exists(h5_file_name)) {
    file_system::rm(h5_file_name, true);
  }
...
// [[OutputRegex, Mismatch in the size of the read dataset. Read \(6,4\) but
// have stored \(3,4\). This means that another thread or process is writing
// data at the same time it is being written by this process.]]
SPECTRE_TEST_CASE("Unit.IO.H5.DatErrorAppendImplSizeChange", "[Unit][IO][H5]") {
  ERROR_TEST();
  // We expect this test to segfault, however the error message is still useful,
  // so we pretend all is fine so that the test can pass.
  std::signal(SIGSEGV, [](int) { std::exit(0); });
  const std::string h5_file_name("Unit.IO.H5.DatErrorAppendImplSizeChange.h5");
  const uint32_t version_number = 4;
  if (file_system::check_if_file_exists(h5_file_name)) {
    file_system::rm(h5_file_name, true);
  }
