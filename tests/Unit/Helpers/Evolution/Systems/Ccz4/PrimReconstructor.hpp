...
compute_prim_solution(
  ...
  using b = ::Ccz4::Tags::b<DataVector, 3>;
  ...
  for (size_t j = 0; j < 3; ++j) {
    ...
    get<b>(vars).get(j) += 1.0e-2 * static_cast<double>((j + 2) + 110);
  }
...
compute_prim_solution_for_second_deriv(
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    for (size_t j = 0; j < 3; ++j) {
      ...
      get<b>(vars).get(j) += square(coords.get(i)) + coords.get(i);
    }
  }
  get(get<ConformalFactor>(vars)) += 2.0;
  get(get<TraceExtrinsicCurvature>(vars)) += 15.0;
...
compute_prim_solution_for_Minkowski(
  ...
  get<::Ccz4::Tags::b<DataVector, 3>>(evolved_vars) =
