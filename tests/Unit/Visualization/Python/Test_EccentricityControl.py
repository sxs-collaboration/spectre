...
from spectre.Informer import (  # Paths for unit test directories
...
class TestEccentricityControl(unittest.TestCase):
    ...
    def create_yaml_file(self):
        try:
            ...
            logging.error(f"An error occured while creating the YAML file: {e}")
    ...
    def test_h5_file_created_correctly(self):
        try:
            # Open the file and check for the existence of the two datasets
            with spectre_h5.H5File(self.h5_filename, "r") as h5_file:
                dataset_AhA = h5_file.get_dat(
                    "ApparentHorizons/ControlSystemAhA_Centers"
                )
                self.assertIsNotNone(
                    dataset_AhA, "Dataset AhA was not found in the HDF5 file."
                )
                h5_file.close_current_object()
                dataset_AhB = h5_file.get_dat(
                    "ApparentHorizons/ControlSystemAhB_Centers"
                )
                self.assertIsNotNone(
                    dataset_AhB, "Dataset AhB was not found in the HDF5 file."
                )
        except Exception as e:
            logging.error(
                f"An error occured while testing HDF5 file creation: {e}"
            )
