...
template <class F, class T, class... ArgumentTypes, size_t... ArgumentIs>
void check_with_random_values_impl(
    ...
    std::array<std::uniform_real_distribution<>, sizeof...(ArgumentTypes)>
        distributions,
    const T& used_for_size, tmpl::list<ArgumentTypes...> /*argument_types*/,
    std::index_sequence<ArgumentIs...> /*index_argument_types*/) noexcept {
  // Note: generator and distributions cannot be const.
  using f_info = tt::function_info<F>;
  ...
  (void)std::initializer_list<char>{(
      ...
      '0')...};
  CHECK_ITERABLE_APPROX(f(std::get<ArgumentIs>(args)...),
                        pypp::call<ResultType>(module_name, function_name,
                                               std::get<ArgumentIs>(args)...));
}

template <class F, class T, class... ReturnTypes, class... ArgumentTypes,
          size_t... ResultIs, size_t... ArgumentIs>
void check_with_random_values_impl(
    F&& f, const std::string& module_name,
    const std::vector<std::string>& function_names, std::mt19937 generator,
    std::array<std::uniform_real_distribution<>, sizeof...(ArgumentTypes)>
        distributions,
    const T& used_for_size, tmpl::list<ReturnTypes...> /*return_types*/,
  ...
  f(make_not_null(&std::get<ResultIs>(results))...,
    std::get<ArgumentIs>(args)...);
  const auto helper = [&module_name, &function_names, &args,
                       &results](auto result_i) {
 ...
 * generated random values in the range [`lower_bound`, `upper_bound`).
 * The argument `used_for_size` is used for constructing the arguments of `f` by
...
template <size_t NumberOfBounds, class F, class T,
          Requires<not cpp17::is_same_v<
              typename tt::function_info<F>::return_type, void>> = nullptr>
...
void check_with_random_values(
  ...
  TestHelpers_detail::check_with_random_values_impl(
      std::forward<F>(f), module_name, function_name, generator,
      std::move(distributions), used_for_size, argument_types{},
 ...
 * Tests the function `f` by comparing the result to that of the python function
 * `function_name` in the file `module_name`. The function is tested by
 * generated random values in the range [`lower_bound`, `upper_bound`) for each
 * argument. The argument `used_for_size` is used for constructing the arguments
 * of `f` by calling `make_with_value<ArgumentType>(used_for_size, 0.0)`. For
 * functions that return by `gsl::not_null`, the result will be initialized with
 * random values rather than to signaling `NaN`s. This means functions do not
 * need to support receiving a signaling `NaN` in their return argument to be
...
void check_with_random_values(
  ...
  TestHelpers_detail::check_with_random_values_impl(
      std::forward<F>(f), module_name, function_names, generator,
      std::move(distributions), used_for_size, return_types{}, argument_types{},
