  ...
  static auto f(const std::array<size_t,Dim>& coeffs,
    ...
    for(size_t i = 0; i < Dim; ++i) {
      ...
      for(size_t d = 0; d < Dim; ++d) {
        result.get(i) *= pow(x.get(d),coeffs[d]);
      }
    }
    return result;
  }
        ...
        for(size_t d = 0; d < Dim; ++d) {
          if(d == i) {
            if(0 == coeffs[d]) {
              ...
              result.get(i,j) *= coeffs[d]*pow(x.get(d),coeffs[d]-1);
            }
          } else {
            result.get(i,j) *= pow(x.get(d),coeffs[d]);
          }
        }
      }
    }
...
struct Var2 : db::DataBoxTag {
  ...
  static auto f(const std::array<size_t,Dim>& coeffs,
    ...
    for(size_t d = 0; d < Dim; ++d) {
      result.get() *= pow(x.get(d),coeffs[d]);
    }
    return result;
  }
  template <size_t Dim>
      ...
      for(size_t d = 0; d < Dim; ++d) {
        if(d == i) {
          if(0 == coeffs[d]) {
            ...
            result.get(i) *= coeffs[d]*pow(x.get(d),coeffs[d]-1);
          }
        } else {
          result.get(i) *= pow(x.get(d),coeffs[d]);
        }
  ...
  for (size_t a = 0; a < extents[0]; ++a) {
    for (size_t n = 0; n < u.number_of_independent_components; ++n) {
      for (size_t s = 0; s < number_of_grid_points; ++s) {
        u.data()[s + n * number_of_grid_points] = (n + 1) * pow(xi[s], a);
      }
    }
    ...
    for (size_t n = 0;
      ...
      for (size_t s = 0; s < number_of_grid_points; ++s) {
        double expected = (0 == a ? 0.0 : a * (n + 1) * pow(xi[s], a - 1));
        CHECK(du[0].data()[s + n * number_of_grid_points] == approx(expected));
      }
    }
  }
}
...
void test_logical_partial_derivatives_2d(const Index<2>& extents) {
  ...
  for (size_t n = 0; n < u.number_of_independent_components; ++n) {
    for (IndexIterator<2> ii(extents); ii; ++ii) {
      u.data()[ii.offset() + n * number_of_grid_points] =
          (n + 1) * pow(xi[ii()[0]], a) * pow(eta[ii()[1]], b);
    }
  }

  auto du = logical_partial_derivatives<VariableTags, GradientTags>(u, extents);

  for (size_t n = 0;
       n < Variables<GradientTags>::number_of_independent_components; ++n) {
    for (IndexIterator<2> ii(extents); ii; ++ii) {
      double expected_dxi = (0 == a ? 0.0
                                    : a * (n + 1) * pow(xi[ii()[0]], a - 1) *
                                          pow(eta[ii()[1]], b));
      double expected_deta = (0 == b ? 0.0
                                     : b * (n + 1) * pow(xi[ii()[0]], a) *
                                           pow(eta[ii()[1]], b - 1));
      CHECK(du[0].data()[ii.offset() + n * number_of_grid_points] ==
            approx(expected_dxi));
      CHECK(du[1].data()[ii.offset() + n * number_of_grid_points] ==
...
void test_logical_partial_derivatives_3d(const Index<3>& extents) {
  ...
  for (size_t n = 0;
    ...
    for (IndexIterator<3> ii(extents); ii; ++ii) {
      ...
      double expected_deta =
          (0 == b ? 0.0
                  : b * (n + 1) * pow(xi[ii()[0]], a) *
                        pow(eta[ii()[1]], b - 1) * pow(zeta[ii()[2]], c));
      double expected_dzeta =
          (0 == c ? 0.0
      ...
      CHECK(du[1].data()[ii.offset() + n * number_of_grid_points] ==
            approx(expected_deta));
      CHECK(du[2].data()[ii.offset() + n * number_of_grid_points] ==
...
void test_partial_derivatives_1d(const Index<1>& extents) {
  ...
  Variables<VariableTags> u(number_of_grid_points);
  Variables<wrap_tags_in<Tags::d, GradientTags, tmpl::size_t<1>, Frame::Grid>>
  ...
  for (size_t a = 0; a < extents[0]; ++a) {
    ...
    auto du = partial_derivatives<VariableTags, GradientTags, 1, Frame::Grid>(
        u, extents, inverse_jacobian);

    for (size_t n = 0; n < du.size(); ++n) {
      CAPTURE_PRECISE(du.data()[n] - expected_du.data()[n]);
      CHECK(du.data()[n] == approx(expected_du.data()[n]));
    }
...
void test_partial_derivatives_2d(const Index<2>& extents) {
  ...
  Variables<VariableTags> u(number_of_grid_points);
  Variables<wrap_tags_in<Tags::d, GradientTags, tmpl::size_t<2>, Frame::Grid>>
      expected_du(number_of_grid_points);
  for (size_t a = 0; a < extents[0]; ++a) {
    for (size_t b = 0; b < extents[1]; ++b) {
      ...
      auto du = partial_derivatives<VariableTags, GradientTags, 2, Frame::Grid>(
          u, extents, inverse_jacobian);

      for (size_t n = 0; n < du.size(); ++n) {
        CHECK(du.data()[n] == approx(expected_du.data()[n]).epsilon(1.e-11));
      }
    }
  }
}
...
void test_partial_derivatives_3d(const Index<3>& extents) {
  ...
  Variables<VariableTags> u(number_of_grid_points);
  Variables<wrap_tags_in<Tags::d, GradientTags, tmpl::size_t<3>, Frame::Grid>>
      expected_du(number_of_grid_points);
  for (size_t a = 0; a < extents[0]; ++a) {
    for (size_t b = 0; b < extents[1]; ++b) {
      for (size_t c = 0; c < extents[2]; ++c) {
...
SPECTRE_TEST_CASE("Unit.Numerical.LinearOperators.PartialDerivs",
                  "[Numerical][LinearOperators][Unit]") {
  size_t n0 = Basis::lgl::maximum_number_of_pts/2;
  size_t n1 = Basis::lgl::maximum_number_of_pts/2 + 1;
  size_t n2 = Basis::lgl::maximum_number_of_pts/2 - 1;
  const Index<1> extents_1d(n0);
