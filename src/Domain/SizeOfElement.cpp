...
std::array<double, VolumeDim> size_of_element(
    ...
    const tnsr::I<DataVector, VolumeDim>& system_coords) noexcept {
  ...
  // system-coord vector between lower face center and upper face center
  auto center_to_center = make_array<VolumeDim>(0.0);
  for (size_t logical_dim = 0; logical_dim < VolumeDim; ++logical_dim) {
    for (size_t system_dim = 0; system_dim < VolumeDim; ++system_dim) {
      const double center_upper = mean_value_on_boundary(
          system_coords.get(system_dim), mesh, logical_dim, Side::Upper);
      const double center_lower = mean_value_on_boundary(
          system_coords.get(system_dim), mesh, logical_dim, Side::Lower);
      center_to_center.at(system_dim) = center_upper - center_lower;
    }
