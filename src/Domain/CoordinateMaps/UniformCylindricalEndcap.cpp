...
bool is_uniform_cylindrical_endcap_invertible_on_sphere_one(
    const std::array<double, 3>& center_one,
    const std::array<double, 3>& center_two, double radius_one,
    double radius_two, double theta_max_one, double theta_max_two) noexcept {
...
UniformCylindricalEndcap::UniformCylindricalEndcap(
    const std::array<double, 3>& center_one,
    const std::array<double, 3>& center_two, double radius_one,
    double radius_two, double z_plane_one, double z_plane_two) noexcept
    : center_one_(center_one),
      center_two_(center_two),
      radius_one_([&radius_one]() noexcept {
      ...
      radius_two_([&radius_two]() noexcept {
        // The equal_within_roundoff below has an implicit scale of 1,
        // so the ASSERT may trigger in the case where we really
        // want an entire domain that is very small.
      ...
      theta_max_one_([&center_one, &radius_one, &z_plane_one]() noexcept {
        const double cos_theta_max = (z_plane_one - center_one[2]) / radius_one;
        ASSERT(abs(cos_theta_max) < 1.0,
               "Plane one must intersect sphere, and at more than one point");
        return acos(cos_theta_max);
      }()),
      theta_max_two_([&center_two, &radius_two, &z_plane_two]() noexcept {
        const double cos_theta_max = (z_plane_two - center_two[2]) / radius_two;
        ASSERT(abs(cos_theta_max) < 1.0,
               "Plane two must intersect sphere, and at more than one point");
        return acos(cos_theta_max);
      }()) {
...
std::optional<std::array<double, 3>> UniformCylindricalEndcap::inverse(
  ...
  // correct value.
  const auto function_to_zero = [this, &target_coords](double rhobar) noexcept {
  ...
  const auto function_and_deriv_to_zero =
      [this, &target_coords](double rhobar) noexcept {
        const double r_one_cos_theta_one =
            radius_one_ * cos(rhobar * theta_max_one_);
        const double r_two_cos_theta_two =
            radius_two_ * cos(rhobar * theta_max_two_);
        ...
        return std::pair<double, double>(func_to_zero,
                                         deriv_of_func_to_zero);
      };
  ...
  const auto this_is_small_if_rhobar_is_near_unity =
      [this, &target_coords]() noexcept {
            ...
            2.0 * delta_lambda *
                (lambda_if_rhobar_is_unity *
                 ...
                 (target_coords[1] - center_one_[1]) *
                     (center_two_[0] - center_one_[0])) -
            2.0 * temp *
                (-(1.0 - lambda_if_rhobar_is_unity) * radius_one_ *
                     theta_max_one_ * cos(theta_max_one_) -
  ...
  const auto this_is_small_if_rhobar_is_near_zero =
      [this, &target_coords]() noexcept {
        const double denom =
        ...
        const double second_term =
            2.0 * delta_lambda *
                (lambda_if_rhobar_is_zero *
                 ...
                 (target_coords[1] - center_one_[1]) *
                     (center_two_[0] - center_one_[0])) -
            square((1.0 - lambda_if_rhobar_is_zero) * radius_one_ *
                       theta_max_one_ +
  ...
  if (function_at_rhobar_min * function_at_rhobar_max > 0.0) {
    ...
    if (abs(function_at_rhobar_min) / abs(function_at_rhobar_max) < 1.e-10) {
      ...
      try {
        ...
        // Should we print out warning here, or do we assume that failure
        // means that there is no root, and thus inverse fails?
        return {};
      }
    ...
    const double abs_tol = 1.e-15;
    const double rel_tol = 1.e-15;

    try {
      rhobar =
      // NOLINTNEXTLINE(clang-analyzer-core)
      RootFinder::toms748(function_to_zero, rhobar_min, rhobar_max,
                          function_at_rhobar_min,
                          function_at_rhobar_max, abs_tol, rel_tol);
    } catch (std::exception&) {
      // Should we print out warning here, or do we assume that failure
      // means that there is no root, and thus inverse fails?
      return {};
...
void UniformCylindricalEndcap::pup(PUP::er& p) noexcept {
  p | center_one_;
  p | center_two_;
  p | radius_one_;
  p | radius_two_;
...
bool operator==(const UniformCylindricalEndcap& lhs,
                const UniformCylindricalEndcap& rhs) noexcept {
