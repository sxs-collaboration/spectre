...
template <typename T>
std::array<T, 3> UniformCylindricalFlatEndcap::operator()(
...
template <typename T>
tnsr::Ij<T, 3, Frame::NoFrame> UniformCylindricalFlatEndcap::jacobian(
    const std::array<T, 3>& source_coords) const {
  const T& xbar = source_coords[0];
  const T& ybar = source_coords[1];
  const T& zbar = source_coords[2];
...
template <typename T>
tnsr::Ij<T, 3, Frame::NoFrame> UniformCylindricalFlatEndcap::inv_jacobian(
    const std::array<T, 3>& source_coords) const {
