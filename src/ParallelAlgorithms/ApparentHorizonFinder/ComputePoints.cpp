...
bool set_current_iteration_coords(
  ...
  while (alg::any_of(
    ...
    // If so, try reinterpolation up to max_interpolation_retries times
    ++current_iteration->interpolation_retries;

    if (current_iteration->interpolation_retries <= max_interpolation_retries) {
      if (fast_flow.current_iteration() == 0) {
      ...
      // We didn't actually try interpolation here so we added one extra to the
