...
struct VolumeVariables {
  ...
  /// A `Variables` of the  `ah::source_vars` in the volume.
  Variables<ah::source_vars<3>> volume_vars;

  /// A `Variables` of the tensors in the volume that we need to interpolate
  /// onto the horizon.
...
/*!
 * \brief Holds the `ylm::Strahlkorper` aand associated quantities for a single
 * `FastFlow` iteration.
 */
template <typename Fr>
struct Iteration {
  ...
  /*!
   *  \brief `block_coord_holders` holds the list of all points (in block
   *  logical coordinates) that need to be interpolated onto.
   *
  ...
  /*!
   *  \brief Holds the interpolated `Variables` on the
   *  points in `block_coord_holders`. The grid points inside
   *  are indexed according to `block_coord_holders`.
   */
  Variables<ah::vars_to_interpolate_to_target<3, Fr>> interpolated_vars{};
  /*!
   * \brief Keeping track of the indices in `interpolated_vars` that have
