 ...
 * callback for the `ah::ReceiveVolumeData` action (but doesn't send the volume
 * variables again because we already did that). Returns if the current time is
 * ready or not.
 */
...
bool check_if_current_time_is_ready(
  ...
  if constexpr (Parallel::is_in_global_cache<Metavariables,
    ...
    if (domain.is_time_dependent()) {
      ...
      const bool current_time_is_ready = [&]() {
              ...
              return current_time.id <= min_expiration_time
                         ...
                         : std::unique_ptr<Parallel::Callback>(
                               new Parallel::SimpleActionCallback<
                                   ReceiveVolumeData<HorizonMetavars>,
                                   decltype(this_proxy),
                                   LinkedMessageId<double>, ElementId<3>,
                                   Mesh<3>, Variables<ah::source_vars<3>>,
                                   std::optional<std::string>, bool>(
                                   this_proxy, incoming_time,
                                   incoming_element_id, incoming_mesh,
                                   Variables<ah::source_vars<3>>{}, dependency,
                                   true));
            });
      }();

      if (not current_time_is_ready) {
        if (verbose_print) {
          ss << "The current time " << current_time << " is not ready.";
          Parallel::printf("%s\n", ss.str());
        }

        // A callback has been set so that ReceiveVolumeData will
