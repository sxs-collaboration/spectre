...
template <typename Fr, bool ForceInertialFrame = false>
void compute_vars_to_interpolate_to_target_impl(
  ...
  const auto& volume_vars = volume_vars_storage->volume_vars;
  const auto& mesh = volume_vars_storage->mesh;

  const auto& spacetime_metric =
      get<gr::Tags::SpacetimeMetric<DataVector, 3>>(volume_vars);
  const auto& pi = get<gh::Tags::Pi<DataVector, 3>>(volume_vars);
  const auto& phi = get<gh::Tags::Phi<DataVector, 3>>(volume_vars);
  const auto& deriv_phi =
      get<::Tags::deriv<gh::Tags::Phi<DataVector, 3>, tmpl::size_t<3>,
                        Frame::Inertial>>(volume_vars);
...
void compute_vars_to_interpolate_to_target(
  ...
  volume_vars_storage->vars_to_interpolate_to_target.initialize(
      volume_vars_storage->volume_vars.number_of_grid_points());

  const auto& mesh = volume_vars_storage->mesh;
  const auto& block = domain.blocks()[element_id.block_id()];
  if (block.is_time_dependent()) {
    if constexpr (std::is_same_v<Fr, ::Frame::Grid>) {
      ElementMap<3, Fr> map_logical_to_grid{
    ...
    compute_vars_to_interpolate_to_target_impl<Fr, true>(volume_vars_storage);
  }
}
