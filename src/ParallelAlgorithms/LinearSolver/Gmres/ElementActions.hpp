...
// The linear solver may run over a section of the array parallel component,
// i.e. over a subset of the elements. This function retrieves the section
// that the element belongs to, or `Parallel::no_section()` if the solve is not
// restricted to a section.
template <typename ParallelComponent, typename ArraySectionIdTag,
          typename DbTagsList>
auto& get_section([[maybe_unused]] const gsl::not_null<db::DataBox<DbTagsList>*>
  ...
  if constexpr (std::is_same_v<ArraySectionIdTag, void>) {
    ...
    std::optional<Parallel::Section<ParallelComponent, ArraySectionIdTag>>&
        section = db::get_mutable_reference<
            Parallel::Tags::Section<ParallelComponent, ArraySectionIdTag>>(box);
    ASSERT(section.has_value(),
           "Call 'get_section' only on elements that belong to a section. This "
           "is probably a bug, because the other elements should be skipped. "
           "The section is: "
...
struct PrepareSolve {
 ...
 public:
  ...
  static std::tuple<db::DataBox<DbTagsList>&&> apply(
    ...
    auto& section =
        get_section<ParallelComponent, ArraySectionIdTag>(make_not_null(&box));
