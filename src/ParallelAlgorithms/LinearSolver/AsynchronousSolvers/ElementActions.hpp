...
/// The asynchronous algorithm doesn't need to take array sections into account,
/// but we may want to observe residuals per-section. This function returns a
/// string identifying the section, or `std::nullopt` to skip the observation on
/// the element.
template <typename ArraySectionIdTag, typename DbTagsList>
std::optional<std::string> get_section_observation_key(
  ...
  if constexpr (std::is_same_v<ArraySectionIdTag, void>) {
    ...
    return db::get<observers::Tags::ObservationKey<ArraySectionIdTag>>(box);
  }
}
...
struct RegisterObservers {
  ...
  register_info(const db::DataBox<DbTagsList>& box,
    ...
    const std::optional<std::string>& section_observation_key =
        get_section_observation_key<ArraySectionIdTag>(box);
