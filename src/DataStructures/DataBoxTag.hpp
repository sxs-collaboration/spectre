...
/*!
 * \ingroup DataBoxGroup
 * \brief Functional Tags for the DataBox inherit from this type
 *
 * \details
 * Used to mark a type as being a DataBoxTag where the label is a prefix to the
 * DataBoxTag that is a member type alias `tag`.
 *
 * \derivedrequires
 * - type alias `tag` of the DataBoxTag that the function operated on
 * \see DataBox DataBoxTag DataBoxString_t get_tag_name
 */
 ...
 * For any type `T`
 * \code
 * using result = db::TagHasLabel<T>;
 * \endcode
 * \metareturns
 * cpp17::bool_constant
 *
 * \semantics
 * If `T` has a static member variable named `label`, then
 * \code
 ...
 * For any type `T`
 * \code
 * using result = db::TagLabelCorrectType<T>;
 * \endcode
 * \metareturns
 * cpp17::bool_constant
 *
 * \semantics
 * If `T` has a static member variable named `label` of type DataBoxString_t,
 * then
 * \code
 *  * typename result::type = cpp17::bool_constant<true>;
 * \endcode
 ...
 * \return string hold the DataBoxTag's name
 */
template <
    typename Tag,
 ...
 * \tparam Tag the DataBoxTag whose name to get
 * \metareturns
 * value membor of type value_type representing the hash of the DataBoxTag
 */
...
struct compute_item_result_helper<Tag, ArgumentsLs<Args...>> {
  static_assert(
      ...
      R"(list<PassedTags...> >")");
  using type = std::result_of_t<decltype(Tag::function)(
      typename compute_item_result_impl<Args>::type...)>;
};
