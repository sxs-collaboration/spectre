 ...
 * member functions.
 *
 * In numerically solving the TOV equations, there is a singularity at the
 * beginning of the integration interval at the center of the star
 * (where u = v = 0). These appear in the denominator of the TOV equations,
 * so these zeroes cause the derivative to be ill-defined at the center.
 * To avoid this, the code uses a Taylor series expansion of the solution
 * to start the integration at a small radius away from the center.
 *
 * This code uses the first two terms of the Taylor series expansion in the
 ...
 * \f}
 *
 * Where \f$c_s^2\f$ is the sound speed squared at the center.
 *
 * The near-center derivatives are then expanded in powers of \f$H_c - H\f$
 * as such:
 *
 * \f{align}
 ...
 * to estimate a dynamically calculated threshold for the Taylor series
 * expansion, which is used in a conditional statement to determine where the
 * integration of the TOV equations switches from the Taylor series expansion to
 ...
 * we use a fallback threshold from the second-order expansion coefficients
 * instead. Then, the numerical integration of the TOV equations proceeds as
 * normal.
 *
 *
 * With these newly calculated quantities \f$m(r)/r\f$ and \f$\ln(h)\f$,
 * the metric can be constructed as:
 *
 * \f{equation}
