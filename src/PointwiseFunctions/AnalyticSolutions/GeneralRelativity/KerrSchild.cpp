...
KerrSchild::variables(const tnsr::I<DataType, 3>& x, const double /*t*/,
  ...
  TempBuffer<tmpl::list<
      ::Tags::TempI<0, 3, Frame::System, DataType>,
  ...
  auto& deriv_log_r = get<::Tags::Tempi<7, 3, Frame::System, DataType>>(buffer);
  ...
  auto& a_cross_x = get<::Tags::Tempi<12, 3, Frame::System, DataType>>(buffer);
  auto& denom =
      get(get<::Tags::TempScalar<13, DataType>>(buffer));
  auto& r = get(get<::Tags::TempScalar<14, DataType>>(buffer));
  auto& deriv_H = get<::Tags::Tempi<15, 3, Frame::System, DataType>>(buffer);
  auto& null_form = get<::Tags::Tempi<16, 3, Frame::System, DataType>>(buffer);
  auto& deriv_null_form =
      get<::Tags::Tempij<17, 3, Frame::System, DataType>>(buffer);
  auto& lapse_squared =
...

  {
    ...
    for (size_t i = 0; i < 3; ++i) {
      get<gr::Tags::Shift<3, Frame::System, DataType>>(result).get(i) =
  ...
  for (size_t i = 0; i < 3; ++i) {
    get<gr::Tags::SpatialMetric<3, Frame::System, DataType>>(result).get(i, i) =
        1.;
    for (size_t j = i; j < 3; ++j) {  // Symmetry
      get<gr::Tags::SpatialMetric<3, Frame::System, DataType>>(result).get(
  ...
  auto det_and_inverse = determinant_and_inverse(
      get<gr::Tags::SpatialMetric<3, Frame::System, DataType>>(result));

  get(get<gr::Tags::SqrtDetSpatialMetric<DataType>>(result)) =
      sqrt(get(det_and_inverse.first));

  get<gr::Tags::InverseSpatialMetric<3, Frame::System, DataType>>(result) =
      det_and_inverse.second;

  get<gr::Tags::ExtrinsicCurvature<3, Frame::System, DataType>>(result) =
      gr::extrinsic_curvature(
          get<gr::Tags::Lapse<DataType>>(result),
          get<gr::Tags::Shift<3, Frame::System, DataType>>(result),
          get<DerivShift<DataType>>(result),
          get<gr::Tags::SpatialMetric<3, Frame::System, DataType>>(result),
          get<::Tags::dt<gr::Tags::SpatialMetric<3, Frame::System, DataType>>>(
              result),
