...
double get_mask(const tnsr::I<double, 3>& x, const double radius_bound) {
  const double radius = sqrt((x.get(0) * x.get(0)) + (x.get(1) * x.get(1)) +
                             (x.get(2) * x.get(2)));
...
HomogeneousSphere::variables(
  ...
  return {Scalar<DataType>{DataType{electron_fractions_[1] * mask +
                                    electron_fractions_[1] * (1.0 - mask)}}};
}
...
HomogeneousSphere::variables(
  ...
  return {Scalar<DataType>{
      DataType{densities_[1] * mask + densities_[1] * (1.0 - mask)}}};
}
...
HomogeneousSphere::variables(
  ...
  const DataType mask = get_mask(x, radius_);
  return {Scalar<DataType>{
      DataType{temperatures_[1] * mask + temperatures_[1] * (1.0 - mask)}}};
}
