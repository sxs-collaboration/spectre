...
Scalar<DataType> compute_piecewise(
    const tnsr::I<DataType, 3, Frame::Physical>& x, const double shock_position,
...
template <typename DataType>
tnsr::I<DataType, 3, Frame::Physical> compute_piecewise_vector(
    const tnsr::I<DataType, 3, Frame::Physical>& x, const double shock_position,
    const std::array<double, 3>& left_value,
    const std::array<double, 3>& right_value) noexcept {
  return tnsr::I<DataType, 3, Frame::Physical>{
...
KomissarovShock::variables(
    const tnsr::I<DataType, 3, Frame::Physical>& x, const double t,
...
#define INSTANTIATE_SCALARS(_, data)                               \
  template tuples::TaggedTuple<TAG(data) < DTYPE(data)>>           \
      KomissarovShock::variables(                                  \
          const tnsr::I<DTYPE(data), 3, Frame::Physical>&, double, \
...
#define INSTANTIATE_VECTORS(_, data)                                \
  template tuples::TaggedTuple<TAG(data) < DTYPE(data), 3>>         \
      KomissarovShock::variables(                                   \
          const tnsr::I<DTYPE(data), 3, Frame::Physical>&, double,  \
          tmpl::list<TAG(data) < DTYPE(data), 3, Frame::Physical>>) \
          const noexcept;
