...
MagnetizedFmDisk::MagnetizedFmDisk(
  ...
  const auto& spatial_metric =
      get<gr::Tags::SpatialMetric<3, Frame::System, DataVector>>(
          unmagnetized_vars);

  const tnsr::I<double, 3, Frame::System> x_max{
  ...
  const double b_squared_max =
      max(get(dot_product(b_field, b_field, spatial_metric)) /
          ...
          square(get(dot_product(
              b_field,
              get<hydro::Tags::SpatialVelocity<DataVector, 3, Frame::System>>(
                  unmagnetized_vars),
              spatial_metric))));
...
MagnetizedFmDisk::unnormalized_magnetic_field(
    const tnsr::I<DataType, 3, Frame::System>& x) const noexcept {
  auto magnetic_field =
