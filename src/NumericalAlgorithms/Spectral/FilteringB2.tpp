...
void zernike_b2_disk_exponential_filter(
  ...
  for (size_t i = 0; i < num_components; ++i) {
    dgemv_('N', spectral_size_m0, n_r, 1.0, ntm_m0.data(), ntm_m0.spacing(),
           temp.data() + i * num_grid_points, 1, 0.0,
           (*u).data() + i * num_grid_points, 1);
  }
  ...
  for (size_t m = 1; m <= M; ++m) {
    ...
    const auto& ntm_mm =
        Spectral::nodal_to_modal_matrix<Spectral::Basis::ZernikeB2,
            ...
            n_r, m, n_r_max);
    for (size_t i = 0; i < num_components; ++i) {
      dgemm_<true>('N', 'N', spectral_size, 2, n_r, 1.0, ntm_mm.data(),
                   ntm_mm.spacing(),
  ...
  DataVector weights(spectral_space_size);
  {
    ...
    for (size_t n_index = 0; n_index <= n_r_max; n_index += 2, ++index) {
      ...
      weights[index] =
          exp(-alpha * pow(n_i / static_cast<double>(n_order), 2 * half_power));
    }
    for (size_t m_index = 1; m_index <= M; ++m_index) {
      const double w_phi = exp(
          -alpha * pow(static_cast<double>(m_index) / static_cast<double>(M),
                       2 * half_power));
      ...
      for (size_t rep = 0; rep < 2; ++rep) {
        for (size_t n_index = m_index; n_index <= n_r_max;
          ...
          weights[index] = exp(-alpha * pow(n_i / static_cast<double>(n_order),
                                            2 * half_power)) *
                           w_phi;
        }
      }
  ...
  const auto& mtn_m0 =
      Spectral::modal_to_nodal_matrix<Spectral::Basis::ZernikeB2,
          ...
          n_r, 0, n_r_max);
  for (size_t i = 0; i < num_components; ++i) {
    dgemv_('N', n_r, spectral_size_m0, 1.0, mtn_m0.data(), mtn_m0.spacing(),
           (*u).data() + i * num_grid_points, 1, 0.0,
           temp.data() + i * num_grid_points, 1);
  }
  ...
  for (size_t m = 1; m <= M; ++m) {
    ...
    const auto& mtn_mm =
        Spectral::modal_to_nodal_matrix<Spectral::Basis::ZernikeB2,
            ...
            n_r, m, n_r_max);
    for (size_t i = 0; i < num_components; ++i) {
      dgemm_<true>('N', 'N', n_r, 2, spectral_size, 1.0, mtn_mm.data(),
                   mtn_mm.spacing(), (*u).data() + i * num_grid_points + offset,
                   spectral_size, 0.0,
                   temp.data() + i * num_grid_points + j * n_r, n_r);
    }
    j += 2;
    offset += 2 * spectral_size;
  }
...
void zernike_b2_cylinder_exponential_filter(
  ...
  for (size_t m = 1; m <= M; ++m) {
    ...
    for (size_t i = 0; i < num_components; ++i) {
      for (size_t k = 0; k < n_z; ++k) {
        dgemm_<true>(
            'N', 'N', spectral_size_m, 2, n_r, 1.0, ntm_mm.data(),
            ntm_mm.spacing(),
            temp.data() + i * num_grid_points + k * n_r * n_phi + j * n_r, n_r,
            0.0,
  ...
  for (size_t m = 1; m <= M; ++m) {
    ...
    for (size_t i = 0; i < num_components; ++i) {
      for (size_t k = 0; k < n_z; ++k) {
        dgemm_<true>(
            'N', 'N', n_r, 2, spectral_size_m, 1.0, mtn_mm.data(),
            mtn_mm.spacing(),
            (*u).data() + i * num_grid_points + k * spectral_space_size +
                offset,
            spectral_size_m, 0.0,
  ...
  const Matrix z_filter = Spectral::filtering::exponential_filter(
      mesh.slice_through(2), alpha, half_power);
