...
auto apply_callback(
    ...
    -> decltype(T::post_interpolation_callback::apply(box, cache, time),
                bool()) {
  return T::post_interpolation_callback::apply(box, cache, time);
}
...
auto apply_callback(
    ...
    -> decltype(T::post_interpolation_callback::apply(*box, *cache, time),
                bool()) {
  T::post_interpolation_callback::apply(*box, *cache, time);
...
void fill_invalid_points(const gsl::not_null<db::DataBox<DbTags>*> box,
  ...
  if (invalid_indices.find(time) != invalid_indices.end() and
      not invalid_indices.at(time).empty()) {
    db::mutate<Tags::IndicesOfInvalidInterpPoints,
        ...
        [&time](const gsl::not_null<
          ...
          indices_of_invalid_points->erase(time);
        });
  }
}
...
bool call_callback(
  ...
  fill_invalid_points<InterpolationTargetTag>(box, time);

  // Fill ::Tags::Variables<typename
  //      InterpolationTargetTag::vars_to_interpolate_to_target>
  // with variables from correct time.
  db::mutate_apply<
      tmpl::list<::Tags::Variables<
          typename InterpolationTargetTag::vars_to_interpolate_to_target>>,
      tmpl::list<Tags::InterpolatedVars<InterpolationTargetTag>>>(
      [&time](
        ...
        *vars = vars_at_all_times.at(time);
      },
      box);

  // apply_callback should return true if we are done with this
  // time.  It should return false only if the callback
  // calls another `intrp::Action` that needs the volume data at this
  // same time.
  return apply_callback<InterpolationTargetTag>(box, cache, time);
}

/// Frees InterpolationTarget's memory associated with the supplied time.
///
...
void clean_up_interpolation_target(
    const gsl::not_null<db::DataBox<DbTags>*> box, const double time) noexcept {
  // We are now done with this time, so we can pop it and
  // clean up data associated with it.
  db::mutate<Tags::Times, Tags::CompletedTimes,
             Tags::IndicesOfFilledInterpPoints,
             Tags::IndicesOfInvalidInterpPoints,
             Tags::InterpolatedVars<InterpolationTargetTag>>(
      box, [&time](const gsl::not_null<std::deque<double>*> ids,
                   const gsl::not_null<std::deque<double>*> completed_ids,
                   const gsl::not_null<
                       std::unordered_map<double, std::unordered_set<size_t>>*>
                       indices_of_filled,
                   const gsl::not_null<
        ...
        completed_ids->push_back(time);
        ASSERT(std::find(ids->begin(), ids->end(), time) != ids->end(),
               "Time " << time << " does not exist in ids");
        ids->erase(std::remove(ids->begin(), ids->end(), time), ids->end());
        // We want to keep track of all completed times to deal with
        // the possibility of late calls to
        // AddTimesToInterpolationTarget.  We could keep all
        // completed_times forever, but we probably don't want it to get too
        // large, so we limit its size.  We assume that
        // asynchronous calls to AddTimesToInterpolationTarget do not span
        // more than 1000 times.
        if (completed_ids->size() > 1000) {
          completed_ids->pop_front();
        }
        indices_of_filled->erase(time);
        indices_of_invalid->erase(time);
        interpolated_vars->erase(time);
      });
}
...
bool have_data_at_all_points(const db::DataBox<DbTags>& box,
  ...
  const size_t filled_size =
      db::get<Tags::IndicesOfFilledInterpPoints>(box).at(time).size();
  const size_t invalid_size = [&box, &time]() noexcept {
    const auto& invalid_indices =
        db::get<Tags::IndicesOfInvalidInterpPoints>(box);
    if (invalid_indices.count(time) > 0) {
      return invalid_indices.at(time).size();
    }
    return 0_st;
  }();
  const size_t interp_size =
      db::get<Tags::InterpolatedVars<InterpolationTargetTag>>(box)
          .at(time)
...
/// the supplied times.  Changes the InterpolationTarget's DataBox
/// accordingly.
///
/// Returns the times that have actually been newly flagged
/// (since some of them may have been flagged already).
///
/// flag_times_for_interpolation is called by an Action
/// of InterpolationTarget
///
/// Currently one Action calls flag_times_for_interpolation:
/// - InterpolationTargetVarsFromElement (called by DgElementArray)
template <typename InterpolationTargetTag, typename DbTags>
std::vector<double> flag_times_for_interpolation(
    const gsl::not_null<db::DataBox<DbTags>*> box,
    const std::vector<double>& times) noexcept {
  // We allow this function to be called multiple times with the same
  // times (e.g. from each element, or from each node of a
  ...
  // of the times (and after that id has already been removed
  // from `ids`).  If this happens, we don't want to add the
  // times again. For that reason we keep track of the
  // times that we have already completed interpolation on.  So
  // here we do not add any times that are already present in
  // `ids` or `completed_ids`.
  std::vector<double> new_times{};

  db::mutate_apply<tmpl::list<Tags::Times>, tmpl::list<Tags::CompletedTimes>>(
      [&times, &new_times](const gsl::not_null<std::deque<double>*> ids,
...
/// (since some of them may have been flagged already).
///
/// flag_times_as_pending is called by an Action
/// of InterpolationTarget
///
/// Currently one Action calls flag_times_as_pending:
/// - AddTimesToInterpolationTarget (called by Events::Interpolate)
template <typename InterpolationTargetTag, typename DbTags>
std::vector<double> flag_times_as_pending(
    const gsl::not_null<db::DataBox<DbTags>*> box,
    const std::vector<double>& times) noexcept {
  // We allow this function to be called multiple times with the same
  // times (e.g. from each element, or from each node of a
  ...
  db::mutate_apply<tmpl::list<Tags::PendingTimes>,
                   tmpl::list<Tags::Times, Tags::CompletedTimes>>(
      [&times, &new_times](const gsl::not_null<std::deque<double>*> pending_ids,
                           const std::deque<double>& ids,
                           const std::deque<double>& completed_ids) noexcept {
        for (auto& id : times) {
          if (std::find(completed_ids.begin(), completed_ids.end(), id) ==
                  completed_ids.end() and
...
void add_received_variables(
    ...
    const double time) noexcept {
  db::mutate<Tags::IndicesOfFilledInterpPoints,
             Tags::InterpolatedVars<InterpolationTargetTag>>(
      box, [&time, &vars_src, &global_offsets](
               const gsl::not_null<
                   std::unordered_map<double, std::unordered_set<size_t>>*>
                   indices_of_filled,
               const gsl::not_null<std::unordered_map<
...
/// - SendPointsToInterpolator (called by AddTimesToInterpolationTarget
...
auto block_logical_coords(const db::DataBox<DbTags>& box,
  ...
  if constexpr (std::is_same_v<typename InterpolationTargetTag::
    ...
    return ::block_logical_coordinates(
        domain, InterpolationTargetTag::compute_target_points::points(
                    box, tmpl::type_<Metavariables>{}, time));
  }
  ...
  if (maps_are_time_dependent<InterpolationTargetTag>(
    ...
    if constexpr (cache_contains_functions_of_time<Metavariables>::value) {
      ...
      // that functions_of_time are up to date at time.
...
/// corresponding to the supplied block logical coordinates and time.
///
...
void set_up_interpolation(
    const gsl::not_null<db::DataBox<DbTags>*> box, const double time,
