...
///      takes a double as an extra argument. `compute_target_points`
...
///      needs the volume data at this time (such as another iteration of
///      the horizon finder).
///
...
/// time.  If any step of the interpolation procedure ever
...
/// up-to-date for a particular time at one step in the
/// interpolation procedure, all subsequent steps of the interpolation
/// procedure for that same time need not worry about
...
/// time being passed into `block_logical_coordinates`.
///
...
/// requested for a new time (e.g. by
/// `intrp::Events::Interpolate`), the time is added to
/// `Tags::PendingTimes`, which holds a
/// `std::deque<double>`, and represents times that we
/// want to interpolate onto, but for which `FunctionOfTime`s are not
/// necessarily up-to-date.  We also keep another list of
/// times: `Tags::Times`, for which `FunctionOfTime`s
/// are guaranteed to be up-to-date.
///
/// The action `Actions::VerifyTimesAndSendPoints` moves
/// times from `PendingTimes` to `Times` as
/// appropriate, and if any times have been so moved, it
...
/// sequential, (i.e. you cannot interpolate onto one time until
...
///   move all PendingTimes to Times
///   if (sequential) {
///     call block_logical_coords and interpolate for first time.
///     // when interpolation is complete,
///     // Actions::InterpolationTargetReceiveVars will begin interpolation
///     // on the next time.
///   } else {
///     call block_logical_coords and interpolate for all times.
///   }
///   return;
/// }
/// if (FunctionOfTimes are up-to-date for any PendingTimes) {
///   move up-to-date PendingTimes to Times
///   if (sequential) {
///     call block_logical_coords and interpolate for first time.
///     // when interpolation is complete,
///     // Actions::InterpolationTargetReceiveVars will begin interpolation
...
///   } else {
///     call block_logical_coords and interpolate for all times.
///     if (PendingTimes is non-empty) {
///       call myself (i.e. execute VerifyTimesAndSendPoints)
///     }
///   }
/// } else {
///   set VerifyTimesAndSendPoints as a callback for the
///   `domain::Tags::FunctionsOfTime` in MutableGlobalCache, so that
///   once `FunctionsOfTime` are updated, VerifyTimesAndSendPoints
///   is called.
/// }
/// ```
///
/// Note that VerifyTimessAndSendPoints always exits in one of three
/// ways:
...
///   `Times`, and there are no `PendingTimes` left.
///
/// We now describe the logic of the Actions that use
/// VerifyTimesAndSendPoints.
///
/// ##### Actions::AddTimesToInterpolationTarget
///
/// `Actions::AddTimesToInterpolationTarget` is called by
/// `intrp::Events::Interpolate` to trigger interpolation for new
/// times. Its logic is as follows, in pseudocode:
///
/// ```
/// Add passed-in times to PendingTimes
/// if (sequential) {
///   if (Times is empty and
///       PendingTimes was empty before it was appended above) {
///     call VerifyTimesAndSendPoints
///   } else {
///     Do nothing, because there is already a sequential interpolation in
///     progress, or there is a callback already waiting.
///   }
/// } else { // not sequential
///   if (Times is non-empty) {
...
/// time.  For the sequential case, it needs to start
/// interpolating for the next time.  The logic is, in pseudocode:
///
///```
/// if (sequential) {
///   if (Times is not empty) {
///     call block_logical_coordinates and interpolate for next time
///   } else if (PendingTimes is not empty) {
///     call VerifyTimesAndSendPoints
///   }
/// }
///```
...
/// of the horizon finder at the same time, so one might
...
/// time for which the `FunctionOfTime`s have already been
