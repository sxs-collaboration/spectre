...
Mesh<Dim> mortar_mesh(const Mesh<Dim>& face_mesh1,
  ...
  for (size_t i = 0; i < Dim; ++i) {
    ASSERT(
        face_mesh1.basis(i) == SpatialDiscretization::Basis::Legendre and
            face_mesh2.basis(i) == SpatialDiscretization::Basis::Legendre,
    ...
    ASSERT(
        face_mesh1.quadrature(i) == SpatialDiscretization::Quadrature::Gauss or
            face_mesh1.quadrature(i) ==
  ...
  return {mortar_extents.indices(), SpatialDiscretization::Basis::Legendre,
          Dim != 0 ? face_mesh1.quadrature(0)
                   : SpatialDiscretization::Quadrature::GaussLobatto};
}
