 ...
 * Arpack. Used to solve generalized eigenvalue problem by transforming
 * it into a standard eigenvalue problem.
 *
 * This function uses the Arpack routine dnaupd
 ...
 * store the imaginary parts of the eigenvalues. The eigenvectors are stored in
 * the `eigenvector` second index is for iterating over the eigenvector and
 * first index is for iterating over the components of that eigenvector(same as
...
void find_generalized_eigenvalues_arpack(
    ...
    Matrix& eigenvectors_real, Matrix& matrix_a, Matrix& matrix_b,
    size_t number_of_evals_to_find, const double sigma,
