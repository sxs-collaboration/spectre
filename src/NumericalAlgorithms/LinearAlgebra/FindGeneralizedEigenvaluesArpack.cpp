...
void arpack_dnaupd_wrapper(linear_solver_LU& M_LUsolver, Matrix& B_input,
                           const double sigma, const std::string which,
                           const int number_of_evals_to_find,
                           DataVector& eigenvalues_real_part,
                           DataVector& eigenvalues_imaginary_part,
                           Matrix& eigenvectors) {
  int num_rows = B_input.rows();
  int nev = number_of_evals_to_find;
  ...
  int info = 0, ido = 0;
  while (ido != 99) {
    dnaupd_(&ido, &bmat, &num_rows, which.c_str(), &nev, &tol, resid.data(),
  ...
  dneupd_(&rvec, howmny.c_str(), select.data(), dr.data(), di.data(), z.data(),
          ...
          which.c_str(), &nev, &tol, resid.data(), &ncv, V.data(), &ldv,
  ...
  for (size_t i = 0; i < static_cast<size_t>(number_of_evals_to_find); i++) {
  ...
  // of the user to ensure that the variable `number_of_evals_to_find` is large
  // enough to accommodate all the eigenvectors of interest.

  return;
}

void find_generalized_eigenvalues_arpack(DataVector& eigenvalues_real_part,
                                         DataVector& eigenvalues_imaginary_part,
                                         Matrix& eigenvectors,
                                         Matrix& matrix_a, Matrix& matrix_b,
                                         const size_t number_of_evals_to_find,
                                         const double sigma,
                                         const std::string which) noexcept {
  ...
  ASSERT(number_of_evals_to_find == eigenvalues_real_part.size() and
             number_of_evals_to_find == eigenvalues_imaginary_part.size() and
             number_of_evals_to_find == eigenvectors.columns(),
  ...
  arpack_dnaupd_wrapper(matrix_M_LU_solver, matrix_b, sigma, which,
                        number_of_evals_to_find, eigenvalues_real_part,
