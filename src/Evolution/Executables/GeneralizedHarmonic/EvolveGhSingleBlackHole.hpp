...
/// \cond
// We need a LinkedMessageId for the observation time.
namespace Tags {
struct AhObservationTime {
  static std::string name() { return "AhObservationTime"; }
  using type = LinkedMessageId<double>;
};

struct AhObservationTimeCompute : AhObservationTime, db::ComputeTag {
  using argument_tags = tmpl::list<::Tags::Time>;
  using base = AhObservationTime;
...
struct EvolutionMetavars : public GeneralizedHarmonicTemplateBase<3, UseLts> {
  ...
  //         ah::TargetPoints::ApparentHorizon<ApparentHorizon,
  //         ::Frame::Inertial>;
  ...
  struct ApparentHorizon : tt::ConformsTo<ah::protocols::HorizonMetavars> {
    using time_tag = ::Tags::AhObservationTime;
    ...
    using horizon_find_callbacks = tmpl::list<
        ...
        ah::callbacks::ObserveFieldsOnHorizon<::ah::surface_tags_for_observing,
                                              ApparentHorizon>>;
    using horizon_find_failure_callbacks =
        tmpl::list<ah::callbacks::FailedHorizonFind<ApparentHorizon, true>>;

    using compute_tags_on_element =
        tmpl::list<::Tags::AhObservationTimeCompute>;
