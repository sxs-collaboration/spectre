...
static void apply(
    ...
    const tnsr::iiK<DataVector, Dim>& d_field_b,
  ...
  *lapse_times_slicing_condition =
  ...
  // time derivative of the natural log of the conformal factor
 ...
 *
 * \note Different from the first-order system, the laspe
...
void apply(const gsl::not_null<db::DataBox<DbTagsList>*> box) {
  ...
  tnsr::I<DataVector, Dim> contracted_field_d_up;    // temp for eq 18 -20
  Scalar<DataVector> half_conformal_factor_squared;  // temp for eq 25
  ...
  // expressions and identities needed for evolution equations: eq 13 - 27
  Scalar<DataVector> trace_a_tilde;                                 // eq 13
  tnsr::iJJ<DataVector, Dim> field_d_up;                            // eq 14
  tnsr::Ijj<DataVector, Dim> conformal_christoffel_second_kind;     // eq 15
  tnsr::iJkk<DataVector, Dim> d_conformal_christoffel_second_kind;  // eq 16
  tnsr::Ijj<DataVector, Dim> christoffel_second_kind;               // eq 17
  tnsr::ii<DataVector, Dim> spatial_ricci_tensor;  // eq 18 - 20
  tnsr::ij<DataVector, Dim> grad_grad_lapse;       // eq 21
  Scalar<DataVector> divergence_lapse;             // eq 22
  tnsr::I<DataVector, Dim>
      contracted_conformal_christoffel_second_kind;  // eq 23
  tnsr::iJ<DataVector, Dim>
      d_contracted_conformal_christoffel_second_kind;             // eq 24
  tnsr::i<DataVector, Dim> spatial_z4_constraint;                 // eq 25
  tnsr::I<DataVector, Dim> upper_spatial_z4_constraint;           // eq 25
  tnsr::ij<DataVector, Dim> grad_spatial_z4_constraint;           // eq 26
  Scalar<DataVector> ricci_scalar_plus_divergence_z4_constraint;  // eq 27
  // free params
        ...
        detail::apply(
            ...
            make_not_null(
                &get<::Tags::dt<::Ccz4::Tags::GammaHat<DataVector, Dim>>>(
                    *dt_vars_ptr)),  // eq 4f
            make_not_null(&get<::Tags::dt<::Ccz4::Tags::b<DataVector, Dim>>>(
                *dt_vars_ptr)),  // eq 4i
            ...
            get<Tags::b<DataVector, Dim>>(evolved_vars),
            ...
            get<::Tags::deriv<Tags::GammaHat<DataVector, Dim>,
                              tmpl::size_t<Dim>, Frame::Inertial>>(
                cell_centered_Ccz4_derivs),
            get<::Tags::deriv<Tags::b<DataVector, Dim>, tmpl::size_t<Dim>,
                              Frame::Inertial>>(cell_centered_Ccz4_derivs));
      },
      box);
}
