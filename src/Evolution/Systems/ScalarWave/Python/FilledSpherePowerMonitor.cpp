...
py::dict sw_filled_sphere_monitors_to_dict(
    const SwFilledSpherePowerMonitors& monitors) {
  py::dict result{};
  result["Psi"] = filled_sphere_monitor_to_dict(monitors.psi);
...
py::dict sw_b3_power_monitors_from_jacobian(
  ...
  return sw_filled_sphere_monitors_to_dict(
      sw_filled_sphere_power_monitors(make_not_null(&cart_to_sphere_matrix),
                                      sw_vars, mesh, jac_inertial_to_grid));
}

py::dict sw_b3_power_monitors_from_domain(
    const ScalarWave::Tags::Psi::type& psi,
    const ScalarWave::Tags::Pi::type& pi,
    const ScalarWave::Tags::Phi<3>::type& phi, const Mesh<3>& mesh,
    const ElementId<3>& element_id, const Domain<3>& domain, const double time,
    const FuncOfTimeMap& functions_of_time) {
  return sw_b3_power_monitors_from_jacobian(
      psi, pi, phi, mesh,
...
void bind_filled_sphere_power_monitor(py::module& m) {  // NOLINT
  m.def("sw_b3_power_monitors", &sw_b3_power_monitors_from_domain,
