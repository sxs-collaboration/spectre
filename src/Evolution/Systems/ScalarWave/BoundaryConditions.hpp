      ...
      for (auto& external_direction_and_normals : unit_normal_one_forms) {
        ...
        db::mutate<dt_variables_tag>(
            ...
            [
              ...
              // debugPK
              if (debugPK) {
                Parallel::printf("\n\nGoing to set BC for VPsi...\n");
              }
              const auto bc_dt_v_psi =
                  BoundaryConditions_detail::set_bc_when_char_speed_is_negative(
                      ...
                      BoundaryConditions_detail::set_dt_v_psi<
                          typename Tags::VPsi::type,
                          VolumeDim>::apply(VPsiMethod, buffer, vars, dt_vars,
                                            unit_normal_one_form),
                      char_speeds.at(0));
              // debugPK
              if (debugPK) {
                Parallel::printf("Going to set BC for VZero...\n");
              }
              const auto bc_dt_v_zero =
                  BoundaryConditions_detail::set_bc_when_char_speed_is_negative(
                      get<::Tags::Tempi<7, VolumeDim, Frame::Inertial,
                                        DataVector>>(buffer),
                      BoundaryConditions_detail::set_dt_v_zero<
                          typename Tags::VZero<VolumeDim>::type,
              ...
              const auto bc_dt_v_plus =
                  BoundaryConditions_detail::set_bc_when_char_speed_is_negative(
                      get<::Tags::TempScalar<8, DataVector>>(buffer),
                      BoundaryConditions_detail::set_dt_v_plus<
                          typename Tags::VPlus::type,
                          VolumeDim>::apply(VPlusMethod, buffer, vars, dt_vars,
                                            unit_normal_one_form),
                      char_speeds.at(2));
              // copy over the value of bc_dt_v_psi finally set, accounting
              // for the char speed of VPsi. This will be used to set
              // Dt<VMinus>
              ...
              const auto bc_dt_all_u =
                  evolved_fields_from_characteristic_fields(
                      constraint_gamma2, bc_dt_v_psi, bc_dt_v_zero,
                      bc_dt_v_plus, bc_dt_v_minus, unit_normal_one_form);
