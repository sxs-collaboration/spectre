 ...
 * PhD thesis (Cornell)
 *
 * Fix the conservative variables according to the following recipe:
 *
 * - If \f$D < D_\text{cutoff}\f$, set \f$D = D_\text{min}\f$
 * - If \f$\tau < 0\f$, set \f$\tau = 0\f$
 * - If \f$\tilde S^2 > \tilde\tau(\tilde\tau + 2\tilde D)\f$, rescale
 *   \f$\tilde S_i\f$ by the factor
 *
 * \f{align*}
 ...
 * (e.g. VariableFixing::FixToAtmosphere).
 */
template <size_t Dim>
