...
void klein_gordon_rhs_npsi4_divided_by_one_minues_y_squared(
  ...
  auto res = 2 * conj(eth_kg_psi) * dr_u + 2 * conj(ethbar_u) * dr_psi +
...
void klein_gordon_rhs_tau(
    ...
    const SpinWeighted<ComplexDataVector, 0>& bondi_r) {
  *result = 0.5 * one_minus_y * dy_w * dy_kg_psi +
            0.25 * square(one_minus_y) / bondi_r * dy_dy_kg_psi +
            0.5 * one_minus_y * bondi_w * dy_dy_kg_psi +
            0.5 * bondi_w * dy_kg_psi;
}
}  // namespace detail
