 ...
 * additional initialization for the inertial coordinates. The functions that
 * are required to be overriden in the derived classes are:
 * - `InitializeJ<true>::get_clone()`: should return a
 * `std::unique_ptr<InitializeJ<true>>` with cloned state.
 * - `InitializeJ<true>::operator() const`: should take as arguments, first a
 ...
 * The `InitializeJ<true>::operator()` should be const, and therefore not alter
 ...
 * classes are:
 * - `InitializeJ<false>::get_clone()`: should return a
 * `std::unique_ptr<InitializeJ<false>>` with cloned state.
 * - `InitializeJ<false>::operator() const`: should take as arguments, first a
 * set of `gsl::not_null` pointers represented by `mutate_tags`, followed by a
 * set of `const` references to quantities represented by `argument_tags`. \note
 * The `InitializeJ<false>::operator()` should be const, and therefore not alter
 * the internal state of the generator. This is compatible with all known
 * use-cases and permits the `InitializeJ` generator to be placed in the
 * `GlobalCache`.
 */
