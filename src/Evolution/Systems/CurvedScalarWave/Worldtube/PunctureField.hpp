 ...
 * scalar charge on a generic orbit in Schwarzschild/Kerr spacetime.
...
void puncture_field(
    ...
    size_t order, const std::array<double, 3>& bh_spin = {0., 0., 0.},
    const std::string& puncture_type = "Schwarzschild");
 ...
 *
 * \details The appropriate expression can be found in analogy to Eq. (36) of
 * \cite Wittek:2024gxn. For non-geodesic orbits, there are
 * additional contributions, see `acceleration_terms_kerr_0`.
 */
...
/*!
 * \brief Computes the acceleration terms of a puncture/singular field
 * \f$\Psi^\mathcal{P}\f$ of a scalar charge on a generic orbit in Kerr
 * spacetime up to zeroth order in coordinate distance.
 * \details The appropriate expression can be found in analogy to Eq. (37) of
 * \cite Wittek:2024gxn. The values ft, fx, fy, fz are the time, x, y, and z
 * components of the self force per unit mass evaluated at the position of the
 * particle; dt_ft, dt_fx, dt_fy, dt_fz are the respective total time
 * derivatives. The code in this function was auto-generated by generating the
 * full expressions with Mathematica and employing common subexpression
 * elimination with sympy. The mathematica file and generating script can be
 * found at https://github.com/guilara/worldtube-tutorial/personal.
 */
void acceleration_terms_kerr_0(
    ...
    const tnsr::I<double, 3>& particle_position,
    const tnsr::I<double, 3>& particle_velocity,
    const tnsr::I<double, 3>& particle_acceleration, double ft,
    double fx, double fy, double fz, double dt_ft,
    double dt_fx, double dt_fy, double dt_fz,
    double bh_mass, const std::array<double, 3>& bh_spin);
