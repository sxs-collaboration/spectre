 ...
 * \f{align*}
 * S({\tilde E}) = & \alpha \tilde P^{ij} K_{ij} - \tilde S^i \partial_i \alpha,
 * S({\tilde S_i}) = & -\tilde E \partial_i \alpha + \tilde S_k \partial_i
 * \beta^k
 * + \frac{1}{2} \alpha \tilde P^{ij} \partial_i \gamma_{jk},
 * \f}
 *
 ...
 * \f$K^{ij}\f$ is the extrinsic curvature, and \f$\alpha\f$, \f$\beta^i\f$,
 * \f$\gamma^{ij}\f$ are the lapse, shift and 3-metric.
 *
