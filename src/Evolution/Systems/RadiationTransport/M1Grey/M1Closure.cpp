...
void compute_closure_impl(
    ...
    const gsl::not_null<tnsr::II<DataVector, 3, Frame::System>*>
        pressure_tensor,
  ...
  // and we do not differentiate between fluid/system frames
  ...
  tnsr::I<double, 3, Frame::System> H_M(0.);
  // Loop over points
  for (size_t s = 0; s < get(v_sqr).size(); ++s) {
    ...
    if (v_sqr_pt < small_velocity) {
      ...
      for (size_t i = 0; i < spatial_dim; i++) {
        ...
        // Closure assuming that fluid-frame = system frame
    ...
    // go through a more expensive closure calculation
      ...
      // system frame momentum density. This is a decomposition of
