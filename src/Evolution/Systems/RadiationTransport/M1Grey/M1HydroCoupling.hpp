...
void compute_m1_hydro_coupling_impl(
    ...
    gsl::not_null<tnsr::i<DataVector, 3, Frame::Inertial>*> source_i,
 ...
 * \f}
 * with \f$W\f$ the Lorentz factor, \f$u_i\f$ the spatial components of
 ...
 * flux density.
 *
 * The function returns in source_n the energy source and in source_i
 * the momentum source. We write a separate action for these sources to
 * make it easier to switch between implicit / explicit time stepping, as
 * well as to add the source terms to both the fluid and M1 evolutions.
 */
...
struct ComputeM1HydroCoupling<tmpl::list<NeutrinoSpecies...>> {
  ...
  static void apply(
      ...
      const gsl::not_null<db::item_type<Tags::M1HydroCouplingSpatial<
          Frame::Inertial, NeutrinoSpecies>>*>... source_i,
      const db::item_type<Tags::GreyEmissivity<NeutrinoSpecies>>&... emissivity,
      const db::item_type<
      ...
      const db::item_type<Tags::TildeJ<NeutrinoSpecies>>&... tilde_j,
      const db::item_type<Tags::TildeHNormal<NeutrinoSpecies>>&... tilde_hn,
      const db::item_type<
      ...
      const db::item_type<hydro::Tags::LorentzFactor<DataVector>>&
          lorentz_factor,
      const db::item_type<gr::Tags::Lapse<>>& lapse,
      const tnsr::ii<DataVector, 3, Frame::Inertial>& spatial_metric,
      const db::item_type<gr::Tags::SqrtDetSpatialMetric<>>&
...
/// \brief Compute the source terms due to neutrino-matter interactions
///
...
/// Uses:
/// - DataBox: Items in
/// RadiationTransport::M1Grey::ComputeM1HydroCoupling::argument_tags
///
/// DataBox changes:
/// - Adds: nothing
/// - Removes: nothing
/// - Modifies: RadiationTransport::M1Grey::ComputeM1HydroCoupling::return_tags
struct M1HydroCoupling {
  ...
  static auto apply(db::DataBox<DbTagsList>& box,
    ...
    db::mutate_apply<ComputeClass>(make_not_null(&box));
