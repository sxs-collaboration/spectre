...
py::dict filled_sphere_monitor_to_dict(
    const std::array<DataVector, 2>& monitor) {
  py::dict result{};
...
py::dict gh_filled_sphere_monitors_to_dict(
    const GhFilledSpherePowerMonitors& monitors) {
  py::dict result{};
  result["SpacetimeMetric"] =
      filled_sphere_monitor_to_dict(monitors.spacetime_metric);
  result["Pi"] = filled_sphere_monitor_to_dict(monitors.pi);
  result["Phi"] = filled_sphere_monitor_to_dict(monitors.phi);
  return result;
}
...
py::dict gh_b3_power_monitors_from_jacobian(
  ...
  return gh_filled_sphere_monitors_to_dict(
      gh_filled_sphere_power_monitors(make_not_null(&cart_to_sphere_matrices),
                                      gh_vars, mesh, jac_inertial_to_grid));
}

py::dict gh_b3_power_monitors_from_domain(
    const gr::Tags::SpacetimeMetric<DataVector, 3, Frame::Inertial>::type&
        spacetime_metric,
    const gh::Tags::Pi<DataVector, 3, Frame::Inertial>::type& pi,
    const gh::Tags::Phi<DataVector, 3, Frame::Inertial>::type& phi,
    ...
    const Domain<3>& domain, const double time,
    const FuncOfTimeMap& functions_of_time) {
  return gh_b3_power_monitors_from_jacobian(
...
void bind_filled_sphere_power_monitor(py::module& m) {  // NOLINT
  m.def("gh_b3_power_monitors", &gh_b3_power_monitors_from_domain,
