...
struct ImposeConstraintPreservingBoundaryConditions {
 ...
 private:
  ...
  template <size_t VolumeDim, typename DbTags, typename ArrayIndex,
            typename ActionList, typename ParallelComponent,
            typename... InboxTags>
  struct apply_impl<VolumeDim, UPsiBcMethod::AnalyticBc,
                    UZeroBcMethod::AnalyticBc, UPlusBcMethod::AnalyticBc,
                    UMinusBcMethod::AnalyticBc, DbTags, ArrayIndex, ActionList,
                    ParallelComponent, InboxTags...> {
    static std::tuple<db::DataBox<DbTags>&&> function_impl(
      ...
      // Call the dirichlet BC action
      return dg::Actions::ImposeDirichletBoundaryConditions<
          Metavariables>::apply(box, inboxes, cache, array_index, action_list,
                                parallel_component);
    }
  };
