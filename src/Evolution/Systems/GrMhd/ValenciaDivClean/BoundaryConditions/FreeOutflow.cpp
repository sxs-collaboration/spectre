...
std::optional<std::string> FreeOutflow::dg_ghost(
  ...
  //  * v_{ghost}^i
  //  * spatial metric
  //  * sqrt of the determinant of spatial metric
  //  * divergence cleaning field on ghost zone
  Variables<tmpl::list<
      ::Tags::TempScalar<0>, hydro::Tags::SpatialVelocity<DataVector, 3>,
      gr::Tags::SpatialMetric<3>, ::Tags::TempScalar<1>, ::Tags::TempScalar<2>>>
      temp_buffer{vector_size};
  ...
  auto& sqrt_det_spatial_metric = get<::Tags::TempScalar<1>>(temp_buffer);
  ...
  determinant_and_inverse(make_not_null(&sqrt_det_spatial_metric),
                          ...
                          interior_inv_spatial_metric);
  get(sqrt_det_spatial_metric) = 1.0 / sqrt(get(sqrt_det_spatial_metric));
  ...
  ConservativeFromPrimitive::apply(
      ...
      interior_lorentz_factor, interior_magnetic_field, sqrt_det_spatial_metric,
      spatial_metric, exterior_divergence_cleaning_field);

  ComputeFluxes::apply(tilde_d_flux, tilde_tau_flux, tilde_s_flux, tilde_b_flux,
                       tilde_phi_flux, *tilde_d, *tilde_tau, *tilde_s, *tilde_b,
                       *tilde_phi, *lapse, *shift, sqrt_det_spatial_metric,
                       spatial_metric, *inv_spatial_metric, interior_pressure,
