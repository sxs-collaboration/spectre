...
DiffusionMonteCarloParameters::DiffusionMonteCarloParameters()
  ...
  // We integrate using the trapeze rule to find where
  // r(P) = i * 0.001
  const double dr = 1.e-5;
  double r = dr;
  double integrand_low = 0.0;
  double integrand_high = 4. / sqrt(M_PI) * r * r * exp(-r * r);
  ...
  for (size_t i = 1; i < p_pts - 1; i++) {
    while (integral * static_cast<double>(p_pts - 1) < static_cast<double>(i)) {
      ...
      integrand_high = 4. / sqrt(M_PI) * r * r * exp(-r * r);
  ...
  for (size_t nt = 0; nt < tau_pts; nt++) {
    ...
    size_t cF = 0;
    while (cF < p_pts && gsl::at(ScatteringRofP, cF) < TargetRho) {
      cF++;
    }
    if (cF == p_pts) {
      ...
      const double TargetF =
          ((gsl::at(ScatteringRofP, cF) - TargetRho) *
               static_cast<double>(cF - 1) +
           (TargetRho - gsl::at(ScatteringRofP, cF - 1)) *
               static_cast<double>(cF)) /
          (gsl::at(ScatteringRofP, cF) - gsl::at(ScatteringRofP, cF - 1)) /
  ...
  *prefactor_diffusion_time_vector = (*prefactor_diffusion_four_velocity) *
  ...
  for (size_t i = 0; i < prefactor_diffusion_time_vector->size(); i++) {
    ...
    if (fabs((*prefactor_diffusion_time_vector)[i]) < 1.e-10) {
      ...
      (*prefactor_diffusion_time_vector)[i] = 1.0;
      (*prefactor_diffusion_four_velocity)[i] = 1.0;
      continue;
    }
    ...
    // / (1 - (A/B) u_t)
    (*prefactor_diffusion_four_velocity)[i] =
        1.0 / (1.0 - (*prefactor_diffusion_time_step)[i] *
                         (*prefactor_diffusion_four_velocity)[i]);
    // A from Eq. 32 and 31
    // A= (A/B) * B
    ...
    (*prefactor_diffusion_time_step)[i] =
        (get(lapse)[i] * (*prefactor_diffusion_time_vector)[i]) /
            (get(lorentz_factor)[i] * (*prefactor_diffusion_four_velocity)[i]) +
        1.0;
  }
}
...
void diffuse_packet(
  ...
  const size_t idx = packet->index_of_closest_grid_point;
  const double scattering_optical_depth = scattering_opacity * time_step *
                                          get(lapse)[idx] *
  ...
  // i.e. Eq 28
  if (p_diffusion < 1.0) {
    const double nr = p_diffusion * static_cast<double>(
                                        diffusion_params.ScatteringRofP.size());
  ...
  const double sth2 = sqrt(1.0 - cth2 * cth2);
  random_number =
  ...
  const double& cth = gsl::at(fluid_frame_angles, 0);
  const double& sth = gsl::at(fluid_frame_angles, 1);
  const double& cph = gsl::at(fluid_frame_angles, 2);
  const double& sph = gsl::at(fluid_frame_angles, 3);
