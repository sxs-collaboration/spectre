...
void TemplatedLocalFunctions<EnergyBins, NeutrinoSpecies>::
  ...
  if (fluid_frame_energy <= gsl::at(energy_at_bin_center, 0)) {
    ...
    *scattering_opacity_packet =
    ...
    if (fluid_frame_energy >= gsl::at(energy_at_bin_center, EnergyBins - 1)) {
      ...
      *scattering_opacity_packet = gsl::at(
      ...
      size_t ne = 1;
      while (fluid_frame_energy < gsl::at(energy_at_bin_center, ne - 1)) {
        ne++;
      }
      const double inter_coef =
          (fluid_frame_energy - gsl::at(energy_at_bin_center, ne - 1)) /
          (gsl::at(energy_at_bin_center, ne) -
           gsl::at(energy_at_bin_center, ne - 1));
      *absorption_opacity_packet =
          log(std::max(
              gsl::at(gsl::at(absorption_opacity_table, species), ne)[index],
              opacity_floor)) *
              inter_coef +
      ...
      *scattering_opacity_packet =
          log(std::max(
              gsl::at(gsl::at(scattering_opacity_table, species), ne)[index],
              opacity_floor)) *
              inter_coef +
...
void TemplatedLocalFunctions<EnergyBins, NeutrinoSpecies>::evolve_packets(
  ...
  for (size_t p = 0; p < n_packets; p++) {
    ...
    while (dt_end_step > 0.05 * (final_time - initial_time)) {
      ...
      // -ln(r)/K_a*p^t/nu
      // with eps used to avoid NaNs in case the rng draws a 0.
      dt_absorption =
          absorption_opacity > opacity_floor ?
          -log(rng_uniform_eps_to_one(*random_number_generator)) /
          (absorption_opacity)*packet.momentum_upper_t / fluid_frame_energy :
          10.0 * dt_end_step;
      ...
      dt_scattering =
          ...
          -log(rng_uniform_eps_to_one(*random_number_generator)) /
          (scattering_opacity)*packet.momentum_upper_t / fluid_frame_energy :
          10.0 * dt_end_step;
