...
clang_format = re.search(".*\/git-(clang-format[-0-9\.]*)", clang_format).group(
    1
)
...
clang_format_version = re.search(
    "clang-format version ([0-9]+)\.([0-9]+)", str(clang_format_version)
)

if (
    int(clang_format_version.group(1)) < 4
    and int(clang_format_version.group(2)) < 8
):
    print(
        "clang-format version %s.%s is too low. Must have at least 3.8"
        % (clang_format_version.group(1), clang_format_version.group(2))
    )
    sys.exit(0)

output = subprocess.check_output(
    [git_executable, clang_format, "--diff"]
).decode("ascii")
